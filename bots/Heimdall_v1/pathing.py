        ...
        while True:
            ...
            if hit:
                ...
                if non_self:
                    ...
                    # Own tile is the ONLY hit and we are not standing on the
                    # target: every strictly-closer neighbour is builder-blocked.
                    # Under cardinal movement a sidestep is always farther, so
                    # greedy search would stand here forever — two builders
                    # meeting head-on deadlock permanently (Cambridge's diagonals
                    # made sidesteps stay closer, so this never happened). Break
                    # the standoff by dodging to a random free cardinal tile.
                    cx = start_n % width
                    cy = start_n // width
                    dodges = []
