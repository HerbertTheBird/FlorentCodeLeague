...
# The two assigned opening attackers share one symmetry target until the enemy
# core is confirmed, then execute the paired blocker/gunner siege. _atk_index
# selects the pair member's role; normal maps spawn the defenders first.
_atk_bot = False
...
def _log_status():
    ...
    if _reinforcement_enemy_id:
        ...
        state = atk_builder.action
        target = (
            getattr(atk_builder.siege, "target", None)
            if state.startswith("siege")
            else atk_symmetry_target()
        )
