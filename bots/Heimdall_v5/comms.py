...
# Slot 9 carries completed-economy bits; the remaining high slots are available
# for future defensive coordination.
...
def note_sentinel_built() -> None:
    v = rc.read_store(OPENING_ROLE_SLOT)
    count = min(15, sentinel_count() + 1)
    rc.write_store(
        OPENING_ROLE_SLOT,
        (v & ~_SENTINEL_COUNT_MASK) | (count << _SENTINEL_COUNT_SHIFT),
    )
...
# Heimdall v5). A builder's spawn INDEX determines its role: index < NUM_ATTACK -> that
...
def economy_index(builder_id: int):
    """This builder's economy lane (0..NUM_ECON-1), or None."""
    i = _opening_index(builder_id)
    return i - NUM_ATTACK if i is not None and i >= NUM_ATTACK else None


def mark_economy_ready(builder_id: int) -> None:
    """Publish that this economy builder completed its supply route."""
    lane = economy_index(builder_id)
    if lane is None:
        return
    value = rc.read_store(ECONOMY_READY_SLOT)
    rc.write_store(ECONOMY_READY_SLOT, value | (1 << lane))


def economy_ready_count() -> int:
    return (rc.read_store(ECONOMY_READY_SLOT) & 0xFF).bit_count()


def publish_opening_ore(pos) -> None:
    """Tell builder #2 which visible mine its spawn was routed toward."""
    n = pos.x + pos.y * map_info._width
    rc.write_store(OPENING_ORE_SLOT, n + 1)


def opening_ore():
    value = rc.read_store(OPENING_ORE_SLOT)
    if value <= 0:
        return None
    n = value - 1
    if n >= map_info._width * map_info._height:
        return None
