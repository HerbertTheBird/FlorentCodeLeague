...
def ore_route_reaches_core(ore: Position) -> bool:
    """Whether our harvester on ``ore`` has a conveyor line into our core."""
    if not in_bounds(ore):
        return False
    n = ore.x + ore.y * _width
    bit = 1 << n
    mine = _bm_et[_IDX_HARVESTER] & _bm_team[_my_team_idx]
    if not (mine & bit):
        return False
    reaches_core, _ = _compute_route_reaches_core()
    return bool((expand_manhattan(bit) & ~bit) & reaches_core)
