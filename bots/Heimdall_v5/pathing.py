...
class Pathing:
    ...
    def bfs_route(
        ...
        target_mask: int,
        avoid: int | None = None,
        return_path_mask: bool = False,
    ):
        log("bfs route")
        if start_mask & target_mask:
            start_bit = start_mask & target_mask
            start_bit &= -start_bit
            s_idx = start_bit.bit_length() - 1
            result = (
                Position(s_idx % self.width, s_idx // self.width),
                Position(s_idx % self.width, s_idx // self.width),
                0,
            )
            return result + (start_bit,) if return_path_mask else result
        width = self.width
        height = self.height
        if avoid is None:
        ...
        while True:
            ...
            if hit:
                ...
                if chosen_prev is None:
                    return None
                if not return_path_mask:
                    return (start_pos, chosen_prev, i)

                # Recover one exact shortest cardinal route from the recorded
                # BFS layers. Combat placement uses this mask to avoid putting
                # a gunner on a tile reserved for the conveyor line.
                path_mask = start_bit
                path_x, path_y = cx, cy
                layer = i
                while layer > 0:
                    ...
                    for dx, dy, step_cost in self._ROUTE_OFFSETS:
                        px = path_x - dx
                        py = path_y - dy
                        candidate_layer = layer - step_cost
                        if (
                            candidate_layer < 0
                            or candidate_layer >= vl_len
                            or not (0 <= px < width and 0 <= py < height)
                        ):
                            continue
                        prev_bit = 1 << (py * width + px)
                        if visited_layers[candidate_layer] & prev_bit:
                            next_pos = Position(px, py)
                            next_layer = candidate_layer
                            path_mask |= prev_bit
                            break
                    if next_pos is None:
                        return None
                    path_x, path_y = next_pos.x, next_pos.y
                    layer = next_layer
                return (start_pos, chosen_prev, i, path_mask)
    ...
    def move_to(
        ...
        # get_avoid's base mask is terrain-oriented. Movement BFS must also
        # model non-walkable structures throughout the route, not merely rely
        # on rc.can_move for the immediate step. Otherwise a turret occupying a
        # narrow choke looks traversable forever and the builder repeatedly
        # walks into it instead of routing around it (Runestone centre choke).
        ...
        avoid |= map_info._bm_any_building & ~walkable_buildings
        if not avoid_own_gunner:
            # Attack builders may need one turn to cross their own gunner's ray
            # in a narrow passage. bfs_move still treats standing on that ray as
            # urgent, so they cross it rather than camping there. Enemy gunner
            # rays remain in `avoid` unconditionally.
        ...
        if self.stuck_turns > 2 + self.rc.get_id() % 8:
            for d in ALL_DIRS:
                candidate = map_info.pos_add(my_pos, d)
                if not map_info.in_bounds(candidate):
                    continue
                candidate_bit = 1 << (candidate.x + candidate.y * self.width)
                if candidate_bit & map_info._bm_enemy_hard_threat:
                    continue
    ...
    def move_to_adjacent(self, target: Position, avoid_turret: bool = True):
        ...
        for d in ALL_DIRS:
            ...
            # Gunner rays are never an acceptable fallback. Sentinel/launcher
            # threat may still be used as the least-bad one-step option below.
            if bit & map_info._bm_enemy_hard_threat:
                continue
            if bit & threat:
    ...
    def _calculate_conveyor_path_to(
        ...
        result = self.bfs_route(
            start_mask,
            target,
            avoid,
            return_path_mask=return_path_mask,
        )
        if result is None:
            return None
        s_pos, p_pos, dist = result[:3]
        if DRAW_DEBUG:
    ...
    def calculate_conveyor_route(self, start: Position, update: bool = False):
        """Like calculate_conveyor_path, plus a mask of the chosen full route."""
        log("full conveyor route from ", start)
        target, avoid = self._get_conveyor_targets_and_avoid()
