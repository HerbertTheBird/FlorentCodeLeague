...
# Gunners chip through high-value defenses and supply lines. A lone conveyor is
# deliberately worth a shot in v5: severing delivery is part of the attack.
GUNNER_BUILDING_SCORE = [0] * map_info._NUM_ET
GUNNER_BUILDING_SCORE[map_info._IDX_CORE] = 256
GUNNER_BUILDING_SCORE[map_info._IDX_HARVESTER] = 40
GUNNER_BUILDING_SCORE[map_info._IDX_GUNNER] = 96
GUNNER_BUILDING_SCORE[map_info._IDX_SENTINEL] = 112
GUNNER_BUILDING_SCORE[map_info._IDX_LAUNCHER] = 72
GUNNER_BUILDING_SCORE[map_info._IDX_CONVEYOR] = 32
GUNNER_BUILDING_SCORE[map_info._IDX_BARRIER] = 0
GUNNER_BUILDING_SCORE[map_info._IDX_SPLITTER] = 40
...
def _placement_candidates():
    ...
    _ensure_attack_shift_plans()
    my = map_info._my_pos
    my_bit = 1 << (my.x + my.y * map_info._width)
    candidates = (
        map_info._bm_seen
        & ~map_info._bm_any_building
        & ~map_info._bm_env[map_info._IDX_ENV_WALL]
        # A builder bot does not count as a building in the map cache, but the
        # engine still forbids constructing a gunner on its tile. Duel's enemy
        # econ bot camped at (8, 5), causing our attacker at (8, 6) to select
        # that permanently-illegal site every turn. Also exclude our own tile;
        # the current builder is intentionally absent from _bm_friendly_bots.
        & ~(map_info._bm_friendly_bots | map_info._bm_enemy_bots | my_bit)
        & ~map_info._bm_my_gunner_claims
        & ~cant_attack
    )
    if _defensive():
        # Never occupy the exact shortest path reserved for this economy
        # builder's unfinished conveyor line.
        from units.econ_states import route
        candidates &= ~route.planned_route_mask()
    if not candidates:
...
def _defensive() -> bool:
    """All v5 builders may passively target the full known enemy structure set."""
    return False
...
def _round_cache_enemy_inputs():
    ...
    visible_core = enemy_core & map_info._bm_visible
    if _defensive():
        # Defensive builders only answer structures around our core/economy.
        # Loading a fixed map must not send them across the board before their
        # own routes are complete.
        home = map_info.expand_manhattan(map_info._bm_my_core_area, 9)
        my_infra = map_info._bm_team[map_info._my_team_idx] & (
            map_info._bm_et[map_info._IDX_HARVESTER]
            | map_info._bm_et[map_info._IDX_CONVEYOR]
            | map_info._bm_et[map_info._IDX_SPLITTER]
        )
        defense_zone = home | map_info.expand_manhattan(my_infra, 4)
        enemy_team_bm = (
        ...
        # Catalogue coordinates are not contact: the core becomes a gunner
        # target only after its footprint is actually visible.
        enemy_team_bm = (
            map_info._bm_team[enemy_idx] & ~map_info._bm_my_gunner_claims
        )
        enemy_team_bm &= ~enemy_core | visible_core
...
def _enemy_core_facing(pos: Position):
    ...
    by map_info._GUNNER_RAYS. Return None when no unobstructed facing reaches
    any of the four core tiles.
    """
...
_has_placed_turret = False
MAX_SCORE = 9


def has_placed_turret() -> bool:
    return _has_placed_turret


_CHIP_TARGET_INDICES = frozenset({
    map_info._IDX_CORE,
    map_info._IDX_HARVESTER,
    map_info._IDX_GUNNER,
    map_info._IDX_SENTINEL,
...
def active_chip_gunner() -> bool:
    """Whether this attacker's last gunner still has a structure to chip."""
    if last_gunner_pos is None:
        return False
    w, h = map_info._width, map_info._height
    n = last_gunner_pos.x + last_gunner_pos.y * w
    bit = 1 << n
    if not (
        bit
    ...
    for dx, dy in map_info._GUNNER_RAYS[di]:
        x, y = last_gunner_pos.x + dx, last_gunner_pos.y + dy
        if not (0 <= x < w and 0 <= y < h):
            break
        target_n = x + y * w
        target_bit = 1 << target_n
        if map_info._bm_env[map_info._IDX_ENV_WALL] & target_bit:
            break
        if not (map_info._bm_any_building & target_bit):
            continue
        et_idx = map_info._building_et_idx[target_n]
        return bool(
            map_info._bm_team[enemy_idx] & target_bit
            and et_idx in _CHIP_TARGET_INDICES
        )
    return False
...
def score():
    ...
    if not _defensive() and active_chip_gunner():
        _cached_claims = 0
        return 0
    # Let an uncontested sentinel work instead of stacking gunners on its core.
    if (
        not _defensive()
        and units.builder.enemy_core_visible()
        and not units.builder.visible_enemy_builders()
    ):
        from units.atk_states import core_siege
        if core_siege.active_sentinel():
            _cached_claims = 0
            return 0
    _cached_claims = _my_claims()
    # Economy and routes come first. This remains a passive cleanup state once
    # no higher-priority economy or immediate-defense job is available.
    return 3 if _cached_claims else 0


def run():
    global cant_attack, last_gunner_pos, _has_placed_turret
    log("ATTACK")
    ...
    if not _defensive() and units.builder.enemy_core_visible():
