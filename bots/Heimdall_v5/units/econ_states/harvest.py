...
def _my_claims():
    ...
    opening_target = units.builder.opening_ore_target()
    if opening_target is not None:
        target_bit = 1 << (opening_target.x + opening_target.y * w)
        # This route was explicitly reserved at spawn. Do not let later opening
        # builders' proximity claims redirect builder #2 to a different mine.
        return available & target_bit
...
COST_MAP_TTL = 100
_stalled_until: dict[int, int] = {}
_progress_target_n = -1
_progress_best_distance = 1 << 30
_progress_last_round = -1
_progress_last_improved = -1
STALL_TURNS = 16
STALL_RETRY_TURNS = 25


def _temporarily_stalled() -> int:
    current = rc.get_current_round()
    expired = [n for n, until in _stalled_until.items() if until <= current]
    for n in expired:
        del _stalled_until[n]
    mask = 0
    for n in _stalled_until:
        mask |= 1 << n
    return mask
def possible_ore():
    w = map_info._width
...
def harvestable_ore():
    ...
    # Economy builders now select by their own reachable path distance, so do
    # not discard a nearby mine merely because it lies on the contested half.
    return (ore
            & ~map_info._bm_et[map_info._IDX_HARVESTER]
            & ~cant_harvest
            & ~_temporarily_stalled())


def _making_approach_progress(ore: Position, targets: set[Position]) -> bool:
    """Yield an ore temporarily after repeated safe-path dodging/no progress."""
    global _progress_target_n, _progress_best_distance
    global _progress_last_round, _progress_last_improved
    current = rc.get_current_round()
    n = ore.x + ore.y * map_info._width
    my = map_info._my_pos
    distance = min(
        (abs(my.x - p.x) + abs(my.y - p.y) for p in targets),
        default=1 << 30,
    )
    # Only consecutive harvest turns count. Defense interruptions should not
    # make a target look stalled when economy resumes later.
    if n != _progress_target_n or _progress_last_round != current - 1:
        _progress_target_n = n
        _progress_best_distance = distance
        _progress_last_improved = current
    elif distance < _progress_best_distance:
        _progress_best_distance = distance
        _progress_last_improved = current
    _progress_last_round = current

    # Once adjacent, waiting for titanium is intentional and must never expire.
    if distance == 0:
        _progress_last_improved = current
        return True
    if current - _progress_last_improved < STALL_TURNS:
        return True
    _stalled_until[n] = current + STALL_RETRY_TURNS
    _progress_target_n = -1
    return False
...
def run():
    ...
    if targets:
        if not _making_approach_progress(best_ore, targets):
            log("harvest approach stalled; trying another ore", best_ore)
            return
        nav.move_to(targets)
    ...
    if rc.can_build_harvester(best_ore) and rc.get_global_resources() >= rc.get_harvester_cost() + map_info.builder_ti_reserve():
        ...
        # Route owns the completion threshold and exact reserved path. Record
        # the original mine-to-core length only once the mine actually exists.
        from units.econ_states import route
        route.note_route_started(path[2])
        map_info.update_at(best_ore)
