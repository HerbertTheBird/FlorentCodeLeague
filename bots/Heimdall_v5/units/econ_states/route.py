...
# Per-builder route plan. Python module state is isolated per unit, so this
# records the line owned by this economy builder without cross-builder clashes.
_planned_total_length = 0
_plan_cache_key = None
_planned_route_mask = 0
_planned_remaining = None
...
def note_route_started(total_length: int) -> None:
    """Remember the original mine-to-core route length for the 10% cutoff."""
    global _planned_total_length, _plan_cache_key
    if total_length is not None and total_length > 0:
        _planned_total_length = max(_planned_total_length, total_length)
    _plan_cache_key = None


def _refresh_route_plan(claims: int | None = None) -> None:
    """Cache the chosen unfinished route, its remaining length, and tile mask."""
    global _plan_cache_key, _planned_route_mask, _planned_remaining
    global _planned_total_length

    if claims is None:
        claims = _my_claims()
    key = (rc.get_current_round(), map_info._struct_version, claims)
    ...
    w = map_info._width
    while candidates:
        candidate, _distance = nav.closest(candidates)
        if candidate is None:
            return
        bit = 1 << (candidate.x + candidate.y * w)
        is_harvester = bool(map_info._bm_et[map_info._IDX_HARVESTER] & bit)
        route_plan = nav.calculate_conveyor_route(
            candidate,
            update=not is_harvester,
        )
        if route_plan is not None:
            _planned_remaining = route_plan[2]
            _planned_route_mask = route_plan[3]
            # Conservative fallback for a route inherited from another builder:
            # it still gets the absolute four-conveyor completion window.
            _planned_total_length = max(
                _planned_total_length,
                _planned_remaining,
            )
            return
        candidates &= ~bit
...
def planned_route_mask() -> int:
    """Tiles reserved for this builder's current shortest conveyor route."""
    if not units.builder._economy_builder:
        return 0
    _refresh_route_plan()
    return _planned_route_mask


def should_finish_before_defense() -> bool:
    """Whether only the final max(4, ceil(10%)) conveyors remain."""
    if not units.builder._economy_builder:
        return False
    _refresh_route_plan(_cached_claims)
    if _planned_remaining is None or _planned_remaining <= 0:
        return False
    completion_limit = max(4, (_planned_total_length + 9) // 10)
    return _planned_remaining <= completion_limit
