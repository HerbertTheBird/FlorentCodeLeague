...
its goal. Economy bots are sent toward the closest titanium ore by the core's
BFS conveyor distance, skipping ores already assigned to earlier lanes.
...
def attack_dest(attackable):
    """Opening throw for the chip attacker: outward, but still on our side."""
    own = map_info._my_core
    enemy = _enemy_core()
    if own is None or enemy is None:
        return None
    ox, oy = own.x + 1, own.y + 1
    ex, ey = enemy.x, enemy.y
    total = max(abs(ex - ox), abs(ey - oy), 1)
    steps = min(4, total)
    tx = ox + (ex - ox) * steps // total
    ty = oy + (ey - oy) * steps // total
    w = map_info._width
    return _pick(
        attackable,
        lambda n: abs(n % w - tx) + abs(n // w - ty),
    )
...
def ensure_launcher_or_wait(rc) -> bool:
    ...
    Only the FIRST economy unit (lane 0) may build the launcher. It is spawned
    ...
    is_first = builder._economy_builder and builder._economy_index == 0
