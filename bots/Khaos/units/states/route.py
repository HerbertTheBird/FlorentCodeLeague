...
def _build_preferred_conveyor(pos: Position, direction: Direction) -> EntityType:
    if _prefer_armoured_conveyor():
        ...
        return map_info._ET_ARMOURED_CONVEYOR
...
def run():
    ...
    if target_conveyor[0].distance_squared(target_conveyor[1]) == 1:
        ...
        if tc1_zone & map_info._bm_enemy_bots:
            near_enemy = True
    if map_info.type_at(target_conveyor[0].x, target_conveyor[0].y) == map_info._ET_ROAD and map_info.team_at(target_conveyor[0].x, target_conveyor[0].y) != map_info._my_team:
        ...
        if rc.can_fire(target):
            ...
            map_info.update_at(target)
        return
    if near_enemy and not (map_info.team_at(target_conveyor[1].x, target_conveyor[1].y) == rc.get_team() and map_info.type_at(target_conveyor[1].x, target_conveyor[1].y) != map_info._ET_MARKER) and (map_info.team_at(target_conveyor[0].x, target_conveyor[0].y) == rc.get_team() and map_info.type_at(target_conveyor[0].x, target_conveyor[0].y) != map_info._ET_MARKER):
