...
def _scan_ray(direction, attackable, allow_builder_bots: bool,
    ...
    while map_info.in_bounds(cur) and cur in attackable:
        ...
        if rc.get_team(bid) == my_team:
            return None
        # Tyr steals/reroutes enemy harvesters instead of spending ammunition
        # destroying them.  A harvester is still a real LOS obstruction, so do
        # not scan through it for a target behind it and do not rotate toward it.
        if bid_etype in (EntityType.HARVESTER, EntityType.GUNNER, EntityType.SENTINEL):
            return None
