...
# One ferry throw per launcher, ever. Module globals are per-unit in this
# engine, so this is exactly "this launcher has spent its throw".
_ferried = False
...
def _try_ferry_ally_forward() -> bool:
    """Throw ONE builder, once, as far toward the enemy core as legal.

    This is the payload half of units/states/ferry.py. `launch` costs no
    titanium and no ammo, and IGNORES WALLS AND LINE OF SIGHT -- it is a
    teleport, not a projectile -- so on a walled map a five-tile throw can be
    worth far more than five tiles of walking. That is the whole point: the
    enemy core's eight-tile delivery ring is untouched at round 25 and only
    fully ours around round 125, and nearly all of that hundred rounds is
    travel.

    Exactly one throw per launcher, ever, and no conditions on which builder.
    Throwing every eligible builder every round would keep flinging defenders
    away from a core that needs them -- the launcher cannot tell a builder
    heading out to rush from one holding station at home, and a defender is
    worth far more than the handful of turns a throw saves. One builder given a
    head start is the whole idea; a conveyor belt of them is a liability.

    Enemy removal still comes first: run() tries _try_throw_enemy_to_history and
    _try_throw_enemy_away before this, and both return early. A thrown enemy
    loses a turn and its position; a thrown ally only saves us a few steps.
    """
    global _ferried
    if _ferried:
        return False
    ...
    best_tile = None
    best_gain = 0
    for bot_tile in rc.get_nearby_tiles(defense.PICKUP_R2):
        bot_id = rc.get_tile_builder_bot_id(bot_tile)
        if bot_id is None or rc.get_team(bot_id) != my_team:
            continue
        here = bot_tile.distance_squared(their)
        for tile in rc.get_nearby_tiles(defense.THROW_R2):
            gain = here - tile.distance_squared(their)
            if gain <= best_gain:
                continue
            if not rc.can_launch(bot_tile, tile):
                continue
            best_bot, best_tile, best_gain = bot_tile, tile, gain

    if best_bot is None:
        return False
    rc.launch(best_bot, best_tile)
    _ferried = True
    log(f"ferry: threw {best_bot} -> {best_tile} toward {their} (one-shot)")
    return True
...
def run():
    ...
    if _try_throw_enemy_away():
        return
    # Nothing hostile in reach: spend the free throw moving our own work
    # forward. Enemy removal always outranks it -- a thrown enemy is denied a
    # turn, a thrown ally only saves us a few.
    if _try_ferry_ally_forward():
        return
