...
def update_at(pos: Position) -> None:
    ...
    if _bm_seen & bit:
        # We already "know" this tile. If this is the first time we have
        # actually looked at it, that knowledge came from somewhere other than
        # our own eyes -- the hardcoded map table, or the symmetry solver -- and
        # this is the one moment it can be checked. Only the table is worth the
        # extra get_tile_env: a wrong symmetry corrects itself, a wrong map does
        # not. See hardcode.verify_tile.
        if not (_bm_seen_observed & bit):
            _bm_seen_observed |= bit
            if hardcode.state == hardcode.ACTIVE:
                hardcode.verify_tile(n, _ENV_INT[get_tile_env(pos)])
    else:
        _bm_seen_observed |= bit
...
def update(recompute: bool = True) -> None:
    ...
    # Try to recognise the board before the symmetry solver runs: if the map is
    # in the table, it settles symmetry itself with better evidence, and the
    # solver's whole-board mirror pass below becomes dead work.
    if hardcode.state == hardcode.SEARCHING:
        hardcode.consider()
