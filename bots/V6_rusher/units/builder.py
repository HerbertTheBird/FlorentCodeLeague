...
states = tuple(sorted(
    [explore, disrupt, harvest, route, heal, attack, defend, cut, rusher],
...
def run():
    ...
    if not _first_run_done:
        ...
        if current_round == 1:
            # Exactly one builder per game reaches its first turn on round 1
            # (verified: 1 of 5-7 on saga, hive, quarry and vault). That makes
            # this the clean place to designate the single core rusher -- see
            # units/states/rusher.py for what it then does and why it is only
            # ever one builder.
            #
            # It takes the slot that used to latch _stay_near_core, which
            # confined that same builder to r^2=100 around our core for the
            # whole match. The two are mutually exclusive by construction: the
            # rusher's entire job is to leave.
