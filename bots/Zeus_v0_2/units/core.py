from fcode import Controller, Direction, Position, GameConstants
import comms
import map_info
...
def run():
    ...
    # and keep the pool topped up to one shot per gunner plus a one-shot buffer:
    # GUNNER_AMMO_COST * (gunners + 1). (Reads the engine constant so it tracks
    # the ammo-per-shot balance automatically.)
    ...
    ammo_target = GameConstants.GUNNER_AMMO_COST * (allied_gunners + 1)
    if ammo < ammo_target:
