...
def _plan_tile_blocked(pos) -> bool:
    """True when a plan tile can NEVER be built on again: a wall, or occupied by a
    building that is not the planned conveyor (ours or the enemy's).

    `_plan_next_action` only ever tested `_my_conveyor_at(pos)` for completion, so
    a plan step whose tile had been taken by something else was returned again
    every single turn, forever: `score()` kept answering NORMAL_SCORE, and
    `_run_plan_action` fell through to `nav.move_adjacent(pos)`, which no-ops
    because the builder is already standing next to it. Nothing abandoned the plan
    -- `builder.run`'s old `conveyor_plan = None` cancel was replaced by a bare
    `pass` on the stated assumption that "_plan_next_action re-checks every step
    against the live map, so a truly stale plan just yields nothing on its own".
    It does not; it yields the same blocked step indefinitely.

    Measured over 16 games vs Champion_v50: 2,474 dead builder-turns, 18.8% of all
    idle turns. On sprint, builder #7 sat at (4,1) printing the same
    "ROUTE-PLAN / move adjacent (4,2)" from turn 201 to turn 999 -- (4,2) held our
    OWN gunner, built on turn 5, so `can_build_conveyor` there was False for the
    entire match.

    Deliberately NOT the sibling bot's test. herbert21 skips on
    `not rc.can_build_conveyor(pos, facing)`, but that is False merely for being
    out of reach -- it requires the tile be orthogonally adjacent to the builder.
    Porting it here would make the plan purely opportunistic and destroy this
    bot's plan-walking, which herbert21 does not do. This asks the narrower
    question: is the tile permanently taken?
    """
    n = pos.x + pos.y * map_info._width
    if (map_info._bm_env[map_info._IDX_ENV_WALL] >> n) & 1:
        return True
    return map_info._building_et_idx[n] >= 0
...
def _plan_next_action():
    ...
    for i in range(n):
        ...
        if _my_conveyor_at(pos):
            ...
            continue
        if _plan_tile_blocked(pos):
            continue                    # tile is gone for good -- don't jam behind it
        ...
        if i + 1 < n:
            ...
            if not _my_conveyor_at(npos) and not _plan_tile_blocked(npos):
