...
from _config import RESERVE_MODE, RESERVE_HEALERS
...
# Titanium every titanium-spending action other than a HEAL must leave in the
# bank. bird1 sizes it for healing, not for a defender spawn: a heal costs 1 Ti
# and restores 4 HP, so 8 Ti is one round of healing from eight builders standing
# on a besieged core. Running the bank to zero is what turns a survivable siege
# into a loss -- the builders are already in position and simply cannot act.
#
# UNSCALED, deliberately. alphaduck returned int(cap * scale_percent / 100), so
# the reserve grew with every building we put down (~10 at round 0, well past 30
# by the time a real economy exists) even though the thing it pays for -- a heal
# -- costs a flat 1 Ti forever. The floor now tracks what it actually funds.
#
...
TI_RESERVE_CAP = 8
# Tried and rejected (on the old scaled reserve): scaling it to current titanium
...
# While a siege is live the rusher is the only unit whose spending advances the
# plan, and it is the unit furthest from the bank. Measured on saga: the rusher
# reached its siege tiles with 279 Ti available and, by the time it was in
# position, home builders had spent the balance down to 8 on their own gunners --
# so it stood beside a purple tile it could not afford for the rest of the game.
# Everyone EXCEPT the rusher therefore leaves one sentinel's worth of titanium on
# top of the heal reserve whenever a siege is running.
SIEGE_RESERVE = 30


def _heal_floor() -> int:
    """Titanium that must stay in the bank so every healer can heal every round
    until the next passive titanium tick.

    A heal costs 1 Ti and is deliberately EXEMPT from the reserve -- the reserve
    exists to fund it. That exemption is what made a flat floor useless: eight
    healers spend 8 Ti a round, so a flat 8 buys exactly ONE round and then the
    bank is empty and nobody can heal. Measured: both glacierkeep losses in the
    86-game panel ended with us on 10 Ti and 0 Ti, at turns 40 and 43.

    Passive income is the one stream that cannot be harvester-killed or besieged:
    PASSIVE_TITANIUM_AMOUNT every PASSIVE_TITANIUM_INTERVAL rounds, unconditional.
    So the exact floor is RESERVE_HEALERS x (rounds until that tick) -- 8 Ti the
    round before a tick, 32 Ti the round after one, sliding in between. It is the
    smallest number that never leaves a healer unable to act.
    """
    interval = GameConstants.PASSIVE_TITANIUM_INTERVAL
    if RESERVE_MODE == "flat":
        return TI_RESERVE_CAP
    if RESERVE_MODE == "passive_tick":
        rnd = _rc.get_current_round()
        return RESERVE_HEALERS * (interval - (rnd % interval))   # 1..interval

    # "survive_rush": hold back enough to keep RESERVE_HEALERS healing for as long
    # as the enemy can afford to keep SHOOTING.
    #
    # Their titanium buys two things: the sentinels (c_build) and the ammunition to
    # fire them (5 Ti per sentinel per round). Whatever is left after the build is
    # their firing budget, so the siege lasts
    #     rounds = (enemy_ti - c_build(n)) / (5n)
    # and surviving it costs us RESERVE_HEALERS Ti per round for that long. That is
    # the number worth protecting -- a floor of 32 Ti is four rounds of healing
    # against a siege that measured twenty.
    #
    # `a` is ENEMY_SENTINEL_SCALE, fixed. Reading it from our own
    # get_sentinel_cost() (the earlier version) tied the reserve to turrets WE
    # built, which is not evidence about their prices.
    import rush
    from _config import (ENEMY_SENTINEL_SCALE, RESERVE_MAX, ENEMY_START_TI,
                         ENEMY_TI_PER_ROUND, ENEMY_TI_OVERESTIMATE,
                         RUSH_SENTINEL_HP)
    k = RESERVE_HEALERS
    enemy_ti = rush.estimated_enemy_ti(
        _rc.get_current_round(), ENEMY_START_TI,
        ENEMY_TI_PER_ROUND * ENEMY_TI_OVERESTIMATE)
    n, _cost = rush.best_n(RUSH_SENTINEL_HP, rush.HEAL_PER_BUILDER * k,
                           ENEMY_SENTINEL_SCALE)
    if n is None:
        # k healers already out-heal every n they could field: nothing to survive,
        # so fall back to the cheap tick-sized floor.
        return RESERVE_HEALERS * (interval - (_rc.get_current_round() % interval))
    build = rush.c_build(n, ENEMY_SENTINEL_SCALE)
    firing_budget = max(0.0, enemy_ti - build)
    siege_rounds = firing_budget / (rush.OPERATING_COST * n)
    global _projected_siege_rounds
    _projected_siege_rounds = siege_rounds
    return int(min(k * siege_rounds, RESERVE_MAX))


def ti_reserve() -> int:
    """Titanium a non-heal action must leave unspent, so healers can always heal.

    Plus, for every unit that is not the rusher while a siege is live, one
    sentinel's price -- see SIEGE_RESERVE.
    """
    import comms
    base = _heal_floor()
    if comms.siege_active() and not comms.am_rusher():
        return base + SIEGE_RESERVE
    return base


# How long the reserve calculation last projected the siege would run. Written by
# _heal_floor() so callers can price ONGOING costs over the same horizon the heal
# reserve is sized against, rather than inventing a second one.
_projected_siege_rounds: float = 0.0


def projected_siege_rounds() -> float:
    """Rounds the enemy can afford to keep firing, per the last reserve
    calculation. 0.0 until _heal_floor() has run in survive_rush mode."""
    return _projected_siege_rounds


# Titanium per round each of our turrets burns keeping itself firing. Ammunition
# is converted 1:1 from titanium, so this is a real titanium drain:
#   gunner   GUNNER_AMMO_COST (4) every round        (reload 1)
#   sentinel SENTINEL_AMMO_COST (10) every 2 rounds  (reload 2) -> 5/round
_GUNNER_TI_PER_ROUND = GameConstants.GUNNER_AMMO_COST
_SENTINEL_TI_PER_ROUND = GameConstants.SENTINEL_AMMO_COST / 2.0


def my_turret_burn_per_round() -> float:
    """Titanium per round ALL our existing turrets need to keep shooting."""
    mine = _bm_team[_my_team_idx]
    g = (_bm_et[_IDX_GUNNER] & mine).bit_count()
    sn = (_bm_et[_IDX_SENTINEL] & mine).bit_count()
    return g * _GUNNER_TI_PER_ROUND + sn * _SENTINEL_TI_PER_ROUND
...
_gunner_any_cache_version: int = -1
_gunner_any_cache: int = 0


def enemy_gunner_threat_any_rotation() -> int:
    """Tiles an enemy GUNNER could shoot AFTER ROTATING -- the union of all eight
    facings, not just the one it happens to point in now.

    `_bm_enemy_gunner_threat` partitions gunners by their CURRENT facing, which is
    right for "am I being shot this instant" but wrong for "is this a safe place to
    put a 30 Ti building". A gunner rotates for 10 Ti and one turn of cooldown, so
    every tile in this mask is one cheap rotation away from being under fire, and a
    sentinel we plant there is a sitting target for the rest of the game. Sentinels
    are excluded from this treatment on purpose: `rotate()` is gunner-only, so a
    sentinel's facing is frozen at build time and its current-facing ray is its
    whole threat, for good.

    Line of sight is kept exactly as the single-facing version models it -- the ray
    is absorbed by the first occupied tile -- because that is real for a gunner
    (probe: 50/1240 rays firable, needing both a clear ray and a body).
    """
    global _gunner_any_cache_version, _gunner_any_cache
    if _struct_version == _gunner_any_cache_version:
        return _gunner_any_cache
    enemy_idx = 1 - _my_team_idx
    gunners = _bm_et[_IDX_GUNNER] & _bm_team[enemy_idx]
    result = 0
    if gunners:
        ...
        convs = _bm_conveyors
        for di in range(8):
            dm = gunners                       # EVERY gunner, in EVERY direction
            dx, dy = _DIRECTION_DELTAS_I[di]
            length = 3 - di % 2                # cardinal 3, diagonal 2
            shift_mask = _turret_shift_masks.get((dx, dy))
            offset = dx + dy * w
            for _i in range(length):
                if offset > 0:
                ...
                result |= dm & ~blocked
                dm &= ~blocked & ~convs
    _gunner_any_cache_version = _struct_version
    _gunner_any_cache = result
    return result
...
def init(c: Controller):
    ...
    global _hop_mask_cache_key, _hop_mask_cache
    global _gunner_any_cache_version, _gunner_any_cache
    ...
    _gunner_any_cache_version = -1
    _gunner_any_cache = 0
    _hop_mask_cache_key = None
    _hop_mask_cache = 0
...
# --- reach limit: how many conveyors a tile is from the core -----------------
_hop_mask_cache_key: tuple | None = None
_hop_mask_cache: int = 0


def conveyor_hop_mask(max_hops: int) -> int:
    """Bitmask of tiles a conveyor could stand on and still be at most `max_hops`
    conveyors from my core, counting itself.

    Read it as: "if titanium were sitting on this tile, how many conveyors would
    carry it into the core?" A conveyor emptying straight into a core cell is 1.
    My core's own tiles are 0, so `manhattan(conveyor_hop_mask(h))` is the set of
    tiles a HARVESTER (or any source) may sit on -- it feeds the neighbour, and
    the neighbour's count is the whole chain.

    Two seed kinds, and the difference is the point of the function:
      - my core, at 0, from which fresh ground grows one hop per step;
      - every conveyor of mine that already reaches the core, at its REAL chain
        length (`conv_dist_core`), not at its distance as the crow flies. A
        winding 14-hop line that passes close to the core is 14 hops of chain,
        and attaching to it inherits all 14 -- seeding it geometrically would
        hide exactly the runaway networks this cap exists to stop.
    Growth spreads over `_conveyor_sites()`. A conveyor that does NOT reach the
    core is a site like any other (route destroys and re-lays those), but it is
    never a seed, because routing into it would deliver titanium nowhere.

    Cached on everything the walk reads: `_struct_version` (which gates both
    `conv_dist_core` and the building half of `get_avoid`) AND `_bm_seen`, which
    is the other half of `get_avoid(True)`'s own key -- its landlocked-ore term
    moves as we explore, with no building placed and so no struct bump.
    """
    global _hop_mask_cache_key, _hop_mask_cache
    key = (_struct_version, _bm_seen, _bm_my_core_area, max_hops)
    if key == _hop_mask_cache_key:
        return _hop_mask_cache
    reaches, _order = _compute_route_reaches_core()
    dist = conv_dist_core
    # Existing chain bucketed by true hop count; index h holds the conveyors that
    # are exactly h conveyors from the core.
    seeds = [0] * (max_hops + 1)
    m = reaches
    while m:
        b = m & -m
        m ^= b
        d = dist[b.bit_length() - 1]
        if 0 < d <= max_hops:
            seeds[d] |= b
    sites = _conveyor_sites()
    result = _bm_my_core_area
    frontier = _bm_my_core_area
    for hop in range(1, max_hops + 1):
        # An empty seed layer must NOT stop the walk: a chain can dip out of the
        # site set and back (its own tiles are seeds, not sites), so keep going
        # and let a later layer re-open the frontier.
        frontier = ((manhattan(frontier) & sites) | seeds[hop]) & ~result
        result |= frontier
    _hop_mask_cache_key = key
    _hop_mask_cache = result
    return result


def _conveyor_sites() -> int:
    """Tiles a conveyor of mine could occupy, now or after clearing one of my own
    buildings off it.

    Derived from `get_avoid(True)` -- the very mask `bfs_route` paths over -- so
    the reach limit can never refuse a chain the router would happily have laid,
    or admit one it cannot. Two terms are added back, because `get_avoid(True)`
    blanks them for reasons that do not apply here:
      - my conveyors: avoided so a route does not path THROUGH the network, but
        route's `hard_block` branch does destroy and re-lay one in place;
      - the tiles my conveyors output into: avoided for the same reason, and yet
        they are precisely `_bm_dead_end`, i.e. where the next conveyor goes.
        `bfs_route` makes the same exception (`avoid &= ~start_mask`).
    Neither is added back over a wall or another building.
    """
    mine = _bm_conveyors & _bm_team[_my_team_idx]
    buildable = _board_mask & ~_bm_env[_IDX_ENV_WALL] & ~_bm_any_building
    tips = _conveyor_target_tiles(mine) & buildable
    return (_board_mask & ~get_avoid(True)) | mine | tips
