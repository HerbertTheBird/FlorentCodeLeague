...
import comms
from _config import SKIP_TURRET_BUILD
rc: Controller = None
nav: Pathing = None
...
cant_attack = 0
# Turret sites this builder could not reach. Same trap cant_harvest had: set once
# and never cleared, so a site written off because a teammate stood in the corridor
# stayed written off all game. Expires on the same clock.
CANT_ATTACK_RETRY_ROUNDS = 16
_cant_attack_stamp = -1


def _expire_cant_attack():
    global cant_attack, _cant_attack_stamp
    now = rc.get_current_round()
    if now - _cant_attack_stamp >= CANT_ATTACK_RETRY_ROUNDS:
        cant_attack = 0
        _cant_attack_stamp = now
...
def get_best_direction(pos):
    ...
    # Sentinel: best valid-placement direction at pos (equal facings indifferent).
    ...
    for d in range(8):
        ...
        if s > best_s_score:
            best_s_score = s
            best_s_dirs = [directions[d]]
        elif s == best_s_score:
            best_s_dirs.append(directions[d])
    if best_s_dirs:
        best_s_dir = random.choice(best_s_dirs)

    # Gunner: best valid-placement direction at pos (equal facings indifferent).
...
def _ensure_round_cache():
    ...
    # alphaduck overrode this to 0 so a turret could always be bought. bird1 keeps
    # the reserve here too: it is 8 flat, not the old scaled 10-30, so a turret is
    # only ever delayed by 8 Ti, and the heal floor holds for every spend.
...
def score(can_move=True):
    ...
    if _cached_claims:
        ...
        if best is None:
            # Unreachable this turn only -- do not write off every claimed site.
            _cached_claims = 0
        else:
            _cached_best = best
...
def _try_instant_preferred(candidates: int) -> bool:
    ...
    while m:
        ...
        if s > best_score:
            best_score = s
            best_options = [(pos, direction, ttype)]
        elif s == best_score and s > 0:
            best_options.append((pos, direction, ttype))
    if not best_options or best_score <= 0:
        return False
    best_pos, best_dir, best_type = random.choice(best_options)

    # bird5: while we are OUT-HEALING whatever is shooting the core, do not buy
    # turrets at all. A gunner is 20 Ti and a sentinel 30 -- on glacierkeep we put
    # up three gunners and a sentinel at turns 24-27 (90 Ti) and were on 0 Ti by
    # turn 37, with five healers standing on the core ring unable to spend 1 Ti
    # each. That 90 Ti is 90 healer-rounds. When the healing is already winning,
    # a turret buys nothing the healing is not already buying.
    # Do not buy turrets in EITHER direction of the healing race.
    #
    #   heal_race_won  -- we are already out-healing them, so a turret buys
    #                     nothing the healing is not already buying.
    #   core_alarm     -- we are BEING RUSHED, and every titanium spent on a
    #                     counter-turret is titanium the healers do not have.
    #
    # The second clause is the one that matters, and b5_all was missing it: the
    # gate read only heal_race_won, which is FALSE while we are losing the race,
    # so turret buying was suppressed exactly when it was harmless and permitted
    # exactly when it was fatal. Measured on the b5_all panel: sprint died at
    # turn 43 having spent 130 Ti on three gunners, a sentinel and their ammo;
    # drumlin died at turn 42 having spent 136 Ti the same way. That is four more
    # healers, in games decided entirely by healing.
    #
    # A turret cannot heal the core, and a rush is decided at the core.
    if SKIP_TURRET_BUILD and comms.heal_race_won():
        return False

    # Under rush: a counter-turret is allowed ONLY if paying for it -- and for
    # keeping every turret we already own firing -- never touches the heal reserve.
    #
    # Both halves are needed, and both were learned the hard way:
    #   * sprint died on 57 Ti and drumlin on 148, each having spent ~130 Ti on
    #     turrets and their ammunition. Turret money was healer money.
    #   * sweden died on 667 Ti with nine units to their five, because a blanket
    #     "no turrets while rushed" rule refused the turrets that actually kill
    #     the attacker. At 667 Ti nothing was competing.
    #
    # So the test is not wealth, and not the rush: it is whether the FULL cost --
    # this turret's build price plus the per-round ammunition of ALL our turrets
    # across the siege the reserve is already projecting -- fits in the titanium
    # we hold ABOVE the heal reserve. Ammunition is converted 1:1 from titanium
    # and is charged every round, so a turret bought once is a bill that recurs;
    # pricing only the build is what let 130 Ti leak away in games decided by
    # healing.
    if SKIP_TURRET_BUILD and comms.core_alarm():
        heal_reserve = map_info.ti_reserve()
        build_cost = (rc.get_gunner_cost() if best_type == EntityType.GUNNER
                      else rc.get_sentinel_cost())
        rounds = map_info.projected_siege_rounds()
        new_burn = (map_info._GUNNER_TI_PER_ROUND if best_type == EntityType.GUNNER
                    else map_info._SENTINEL_TI_PER_ROUND)
        upkeep = (map_info.my_turret_burn_per_round() + new_burn) * rounds
        if rc.get_global_resources() < heal_reserve + build_cost + upkeep:
            return False
...
def _lookahead_step(candidates: int):
    ...
    for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
        ...
        if s >= threshold and s > best_score:
            best_score = s
            best_steps = [Position(nx, ny)]
        elif s >= threshold and s == best_score:
            best_steps.append(Position(nx, ny))
    return random.choice(best_steps) if best_steps else None
