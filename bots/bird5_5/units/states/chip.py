...
_DIAG = [(1, 1), (-1, 1), (1, -1), (-1, -1)]        # == chip_precompute.DIAGONALS
N_BARRIER = 2
...
def _fire(tpos):
    # A builder attack costs 2 Ti. It is a spend like any other, so it leaves the
    # heal reserve alone -- chipping a barrier is never worth losing a heal.
    if rc.get_global_resources() >= 2 + map_info.ti_reserve() and rc.can_fire(tpos):
        rc.fire(tpos)
