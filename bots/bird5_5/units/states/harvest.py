...
import units.states.rush_attack
import payg
import comms
from log import log
rc: Controller = None
...
def _my_claims(repair=False):
    ...
    # Same reach limit route enforces, and it has to be here too: a harvester is
    # only worth 10 Ti per 4 rounds if something carries that titanium home. Ore
    # that route would later refuse to connect is ore we must not put a harvester
    # on -- that is the stranded-harvester failure the PAYG note in _config.py
    # warns about, arrived at from the other side. A harvester feeds the conveyor
    # BESIDE it, so the neighbour is what must be in reach; the core sits in the
    # mask at 0 hops, so core-adjacent ore (which needs no conveyor at all) passes.
    available &= map_info.manhattan(map_info.conveyor_hop_mask(MAX_ROUTE_CONVEYORS))
    if repair:
...
def run(can_move=True):
    ...
    if rc.can_build_harvester(best_ore) and rc.get_global_resources() >= rc.get_harvester_cost() + map_info.ti_reserve():
        ...
        if reach != -1:
            ...
            map_info.update_at(best_ore)
            # The rusher counts its own opening harvesters: once the quota is met
            # rush_attack starts scoring and pulls it off the economy for good.
            units.states.rush_attack.harvesters_built += 1
