...
from _config import PAYG_HORIZON, MAX_ROUTE_CONVEYORS
...
def _my_claims(repair=False):
    ...
    # Reach limit: refuse anything whose chain to the core would exceed
    # MAX_ROUTE_CONVEYORS conveyors. The two candidate kinds are measured
    # differently, and the difference is one hop:
    #   - a dead end is where the NEXT CONVEYOR goes, so the tile itself must be
    #     within the limit -> intersect with `in_reach`;
    #   - a harvester keeps its tile and feeds a conveyor BESIDE it, so it is the
    #     neighbour that must be within the limit -> intersect with the manhattan
    #     ring of `in_reach`. (`in_reach` contains the core at 0 hops, so ore
    #     touching the core, which needs no conveyor at all, still passes.)
    in_reach = map_info.conveyor_hop_mask(MAX_ROUTE_CONVEYORS)
    candidates &= in_reach
    ...
    if my_harvesters:
        candidates |= my_harvesters & not_blocked() & map_info.manhattan(in_reach)
...
def run(can_move=True):
    ...
    built = False
    # `need` (conveyor cost + reserve) gates this build, not just the destroy
    # above. score()'s affordability test uses nav.conveyor_cost, which is a
    # DISCOUNTED multi-hop estimate (CONVEYOR_COST_DISCOUNT = 0.65) and carries
    # no reserve term -- so passing it did not mean this single conveyor leaves
    # the heal floor intact. This is the last unguarded titanium spend.
    if rc.get_global_resources() >= need and rc.can_build_conveyor(destroy, direction):
