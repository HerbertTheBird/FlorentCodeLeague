...
# Unscaled titanium floor every build must leave behind so heal/attack stay
# affordable. Heal is 4 HP per Ti (HEAL_AMOUNT=4, BUILDER_BOT_HEAL_COST=1), four
# times attack's efficiency and the best use of titanium in the game. Measured:
# of 86 core-adjacent builder-turns with a damaged core, 23 produced no heal and
# 22 of those had ZERO titanium; median bank during core fights was 3 Ti.
# A floor of 10 takes core-heal success from 73.3% to 99.0%. UNSCALED because
# action costs do not scale with build scale. 20 was tested and is worse
# (-0.15 vs V6_earlysiege): a large reserve costs tempo against rush bots.
ACTION_FLOOR = 10
_reserve_armed = False
...
def arm_reserve(threat_now: bool) -> None:
    """Core-side hint that the reserve is needed. Kept for the core's own turn;
    every other unit decides from the broadcast instead -- see ti_reserve."""
    global _reserve_armed
    _reserve_armed = bool(threat_now)


def ti_reserve_base() -> float:
    """Titanium a build action must leave unspent, so an alarm is answerable.

    Held only while the core says it is in trouble, not for the whole game. The
    reserve buys one thing: that the defensive spawn is affordable the round it
    is needed. Holding it unconditionally taxes every build for the entire game
    -- TI_RESERVE_CAP scales with build cost, so about 30 Ti at 300% -- against
    an alarm that on most maps never comes.

    Distress engages at core_vitals.ENGAGE_HP, which against a single sentinel is
    roughly 33 rounds before the core dies, so there is time to accumulate the
    reserve after the signal rather than before it. Each unit reads the signal
    off slot 0, so builders across the map hold back in the same round the core
    does -- `_reserve_armed` alone could never do that, since every unit runs its
    own interpreter and only the core ever calls arm_reserve.

    Measured: removing this gate costs 0.042 in self-play and 0.028 against
    osteo2, so the conditional reserve is doing real work in both directions.
    """
    import comms
    if not (_reserve_armed or comms.core_distress()):
        return ACTION_FLOOR
    return ACTION_FLOOR + int(TI_RESERVE_CAP*_rc.get_scale_percent()/100)
...
_bm_ti_carrying: int = 0       # conveyors believed to carry titanium (within 3 up/downstream of an observed ti conveyor, or 3 downstream of a harvester-fed conveyor)
...
def ground_at(x, y):
    ...
    # threw here on the core's first turn and every turn after, for the whole
    # game. On vase (11x16) that left both players with no builder ever spawned
    # and 0 titanium mined in 1000 rounds.
...
def reachable_from_me() -> int:
    """Tiles this unit can actually WALK to: a flood fill from my own tile over
    `passable()`. Distinct from `passable()` itself, which is merely the set of
    tiles that are not blocked ANYWHERE on the board -- a distinction that
    produced a false "closest() is broken" diagnosis when conflated."""
    my = _my_pos
    start = 1 << (my.x + my.y * _width)
    graph = passable() | start
    w = _width
    nlc = _not_left_col
    nrc = _not_right_col
    seen = start
...
# #42 siege-window reserve: hold a launcher's price for the relay while it crosses.
# Without it the bank drops below a launcher, best_hop's site test (can_build_launcher)
# fails, the relay reports "no hop", stalls and stands down three hops short -- while
# other builders spend the money on 3 Ti barriers. A per-unit reserve cannot work
# (each unit has its own interpreter); the round number is what they all agree on.
SIEGE_WINDOW = -1   # launcher siege disabled -> never reserve for a launcher


def ti_reserve() -> int:
    base = ti_reserve_base()
    try:
        if _rc is not None and _rc.get_current_round() <= SIEGE_WINDOW:
            return base + _rc.get_launcher_cost()
    except Exception:
        pass
    return base
