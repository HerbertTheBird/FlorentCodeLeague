...
# Every reason for writing one off is temporary -- an enemy turret that dies, a
# teammate in the corridor, titanium not yet banked. Observed on antler: ore four
# tiles from our own core sat unharvested for 611 rounds, free of enemy coverage
# from round 200 on, because it had been blacklisted at round 40. We finished
# that game with 2 harvesters built to the opponent's 4.
#
# Retrying costs one path search per ore per RETRY_ROUNDS, and builders use 1.2 ms
# of a 10 ms turn, so the search this was saving was never the constraint.
...
def _coords(mask, limit=6):
    """Compact "x.y|x.y" rendering of a bitmask, for metrics fields."""
    w = map_info._width
    out = []
    m = mask
...
def _find_harvest_target(repair=False):
    ...
    while available:
        ...
        if candidate is None:
            if metrics.ENABLED:
                w2 = map_info._width
                reach = map_info.passable()
                tot_ore = map_info._bm_env[map_info._IDX_ENV_ORE_TI]
                metrics.rej("harvest", "unreachable",
                            n=available.bit_count(),
                            ore_total=tot_ore.bit_count(),
                            ore_possible=possible_ore().bit_count(),
                            ore_harvestable=harvestable_ore().bit_count(),
                            blacklisted=cant_harvest.bit_count(),
                            passable_total=reach.bit_count(),
                            reach_me=map_info.reachable_from_me().bit_count(),
                            claim_adj_reach=(map_info.manhattan(
                                map_info.reachable_from_me()) & available).bit_count(),
                            allore_adj_reach=(map_info.manhattan(
                                map_info.reachable_from_me())
                                & harvestable_ore()).bit_count(),
                            # Decisive: does closest() succeed on the FULL
                            # harvestable set at this same instant? If yes, the
                            # fault is in _my_claims; if no, in closest().
                            closest_allore=(1 if nav.closest(
                                harvestable_ore())[0] is not None else 0),
                            closest_avail=(1 if nav.closest(
                                available)[0] is not None else 0),
                            claims=_coords(available),
                            allore=_coords(harvestable_ore()),
        ...
        if cand_path is None:
            metrics.rej("harvest", "unroutable", at=(candidate.x, candidate.y))
        ...
        if cost > ti:
            metrics.rej("harvest", "too_expensive", at=(candidate.x, candidate.y),
                        cost=int(cost), ti=ti, L=cand_path[2])
            available &= ~cand_bit
            continue
