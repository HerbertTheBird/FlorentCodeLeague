...
def generate_explore_target():
    ...
    # Flood ONLY from my own position, so every tile the fill reaches is
    # reachable BY ME. The old code also seeded every other friendly builder (and
    # interpolated waypoints toward them) to push the frontier away from the
    # crew; the side effect was that the chosen ring could lie in a component
    # only *they* could reach, and the bot then targeted a tile it could never
    # walk to. On vase that produced builders standing still for hundreds of
    # turns (id=3 frozen at (0,9) from round 16 to 999).
    #
    # Spreading is preserved below by subtracting the crew's neighbourhood from
    # the candidate ring instead of by seeding the fill with it -- a preference,
    # not a constraint, so it can never make the target unreachable.
    ...
    ring = recent_frontiers[0]
    # Prefer the far ring, but away from where the rest of the crew already is.
    # `expand_manhattan(..., 3)` is the crew's immediate neighbourhood; if
    # avoiding it would leave nothing, take the ring as-is.
    others = map_info._bm_friendly_bots & ~seeds
    if others:
        spread = ring & ~map_info.expand_manhattan(others, 3)
        if spread:
            ring = spread
    explore_target = _random_tile(ring)
...
def _step_anywhere():
    ...
    threshold is `2 + id % 8`, and instrumenting vase showed the counter peaking
    at 2 (id=8, needs 3) and 5 (id=3, needs 6) -- always short. The result was
    builders that never moved again: 656 failed explore turns to 38 successful
    ones in one game, with id=3 pinned to a single tile from round 16 to 999.

    Stepping somewhere -- anywhere -- breaks the pocket that made every target
    unreachable, and next turn's fill is seeded from the new tile.
    """
