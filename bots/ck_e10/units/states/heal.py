...
def _do_best_heal():
    ...
    if metrics.ENABLED:
        ...
        mp = map_info._my_pos
        if core_area and (map_info.manhattan(core_area)
                          & (1 << (mp.x + mp.y * map_info._width))):
            if best is None:
                metrics.rej("heal", "no_target_at_core",
                            ti=rc.get_global_resources())
            else:
                metrics.act("heal", "core_adj", ti=rc.get_global_resources())
    if best is not None:
        rc.heal(best)
...
def _heal_worthy(mask: int) -> int:
    """The subset of `mask` a heal would actually be spent on.

    `_do_best_heal` is the only thing in this module that heals, and it skips
    anything missing HEAL_MIN_DAMAGE or less because a 4 HP heal costing 1 Ti is
    wasted topping off a 1-2 point dent. Every test that decides where to WALK or
    whether to HOLD has to use the same predicate, or the two disagree -- and the
    disagreement is a builder standing beside two conveyors each missing 1 HP,
    scoring MAX_SCORE (which outranks attack), and healing neither of them, every
...
def _core_distress_target():
    """The core, when it is losing and this builder should answer, else None.

    The verdict comes off the wire (comms.core_distress), not from what this
    builder can see, which is the point: a builder ten tiles away has no vision
    of its own core and `map_info` remembers whatever it last looked at, so
    without the broadcast the bots furthest from the fight are exactly the ones
    that never learn there is one.

    Capped at core_vitals.WANT_HEALERS answering at once -- 2.25 builders hold
    one sentinel off and three gain on it, and past that it is builders queueing
    for tiles around a 2x2 core while the economy pays for it. A builder already
    standing beside the core is kept there rather than counted out by its own
    arrival, which would make it leave and come back every other round.
    """
    distress = comms.core_distress()
    if not distress:
        return None
    core_area = map_info._bm_my_core_area
    if not core_area:
    ...
    adjacent = bool(map_info.manhattan(core_area) & my_bit)
    if not adjacent:
        helpers = (map_info.manhattan(core_area)
                   & map_info._bm_friendly_bots).bit_count()
        if helpers >= core_vitals.WANT_HEALERS:
            return None
    target, dist = nav.closest_within(core_area, max_dist=MAX_HEAL_DIST)
    return target if dist >= 0 else None


def _core_ring():
    # Every tile a builder could stand on that cardinally touches our 2x2 core.

    #   Filtered on `_bm_blocked` -- the same terrain+buildings notion
    #   `pathing.move_adjacent` uses via `is_passable`, with conveyors left walkable.
    #   Deliberately NOT `passable()`, which also strips enemy-launcher adjacency,
    #   enemy bots and parked teammates and would make this a strict SUBSET of the
    #   single-tile approach set exactly when enemies are beside the core.
    # (see build_arms9.py for the full rationale)
    core_area = map_info._bm_my_core_area
    if not core_area:
        return None
    ring = (map_info.manhattan(core_area) & ~core_area
            & map_info._board_mask & ~map_info._bm_blocked)
    my = map_info._my_pos
    ring |= (1 << (my.x + my.y * map_info._width)) & map_info.manhattan(core_area)
    if not ring:
        return None
    out = set()
    w = map_info._width
...
def _find_target():
    ...
    # While the core is still healthy (>= CORE_HEAL_HP) it isn't a real heal
    # target -- drop it from the candidates. score() offers a low-priority idle
    # top-off instead. Below the threshold it heals like anything else.
    ...
    # under active fire used to be included here on the grounds that it was about
    # to get worse; the cost of being wrong was a builder walking over and then
    # standing beside it doing nothing, because _do_best_heal declines to top off
    # a 1-2 point dent. One more turn of damage puts it in `damaged` on its own.
...
def _idle_core_target():
    ...
    if not _heal_worthy(core_area & map_info._bm_damaged & map_info._bm_visible):
        return None                            # nothing a heal would be spent on
    if _core_hp() < CORE_HEAL_HP:
...
def score(can_move=True):
    ...
    if not can_move:
        ...
        _cached_target = None
        return 0
    # A core that is losing, in two tiers, because "hurt" and "about to die" want
    # different answers. CRITICAL outranks attack outright -- nothing else on the
    # board matters if the core dies. ENGAGED only enters at the normal heal
    # priority: it is worth walking to, but not worth preempting a builder that
    # is mid-fight, and treating the two the same cost 0.049 in self-play by
    # pulling three builders off work for every dip below ENGAGE_HP.
    #
    # Either way this is checked BEFORE _find_target, so the core cannot be
    # out-voted by the raw-HP tie-break, under which a conveyor missing 15 of 20
    # always beats a core missing 250 of 500.
    core_target = _core_distress_target()
    if core_target is not None:
        _cached_target = core_target
        return (MAX_SCORE if comms.core_distress() == core_vitals.CRITICAL
                else NORMAL_SCORE)
...
def run(can_move=True):
    ...
    if target is None:
        ...
        return
    log("HEAL", target)
    if map_info._bm_my_core_area & (1 << (target.x + target.y * map_info._width)):
        ring = _core_ring()
        if ring:
            nav.move_to(ring)
            return
