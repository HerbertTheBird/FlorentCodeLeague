...
# #40 FLOW TARGETING. Conveyors were zeroed for BOTH turret types, so a turret
# placed to strangle the enemy core would pour 18 damage a shot into 500 HP of core
# and never touch the belt feeding it. map_info already tracks which conveyors are
# actually carrying (_bm_ti_carrying / conv_load), so a loaded belt tile is the
# thing worth shooting: a conveyor is 20 HP, and cutting a loaded run stops delivery
# outright rather than chipping an HP pool we can never finish.
SENTINEL_BUILDING_SCORE[map_info._IDX_CONVEYOR] = 20
...
GUNNER_BUILDING_SCORE[map_info._IDX_CONVEYOR] = 40
...
# Turret sites this builder could not reach. Same trap cant_harvest had: set
# once and never cleared, so a site written off because a teammate stood in the
# corridor stayed written off all game. Expires on the same clock.
