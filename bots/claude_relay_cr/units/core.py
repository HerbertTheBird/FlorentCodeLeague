...
def run():
    ...
    spawned_conv = False
    # #40 CONV-ROOT SHIFT. The relay hijacks the FIRST builder (id 3/4), which is
    # exactly the round-0 conveyor-tree root -- so its plan was handed to a builder
    # that immediately leaves, and the largest opening tree was simply dropped.
    # Measured at t40 on midgard, team 0: 10 conveyors / 5 harvesters / 210 collected
    # against herbert14's 30 / 8 / 350. Shifting the schedule one round lets round 0
    # spawn the relay builder and starts the conv roots at round 1, so the trees go
    # to builders that stay. Tyr does the equivalent by spawning its siege builder
    # deliberately on its own start tile.
    if 0 < r <= num_groups:
