...
def run():
    ...
    if rc.get_id() <= 4 and rc.get_current_round() <= 60:
        import relaygeom as _rg
        _mc = _rg.my_core(); _tc = _rg.their_core()
        _p = map_info._my_pos
        print("DBG r=%-3d u=%d pos=(%d,%d) state=%-9s sym[h=%d v=%d r=%d solved=%d] mycore=%s theircore=%s seen_tc=%s dist=%s" % (
            rc.get_current_round(), rc.get_id(), _p.x, _p.y,
            best_state.__name__.rsplit(".",1)[-1] if best_state else "none",
            map_info._hor_sym, map_info._ver_sym, map_info._rot_sym, map_info._solved_sym,
            _mc, _tc, map_info._their_core is not None,
            _rg.dist_at((_p.x,_p.y)) if _tc else "-"), flush=True)
