...
def _publish_reserve(active: bool) -> None:
    """Reserve one launcher while we still need one; release it when done."""
    try:
        map_info.set_siege_reserve(rc.get_launcher_cost() if active else 0)
    except Exception:
        pass
...
def _dbg(*a):
    if rc.get_current_round() <= 30 and rc.get_id() <= 4:
        print("RLY r=%d u=%d" % (rc.get_current_round(), rc.get_id()), *a, flush=True)
...
def run(can_move=True):
    ...
    if near is not None:
        ...
        if _hold_turns <= HOLD_PATIENCE:
            ...
            _dbg("HOLD beside", near, "hold_turns", _hold_turns)
            return
        _dbg('IGNORING launcher', near)
    _ignored.add(near)
    ...
    if not has_op() or rc.get_global_resources() < rc.get_launcher_cost():
        ...
        _holding = True
        _stall += 1
        _dbg("WAIT money: op", has_op(), "ti", rc.get_global_resources(), "cost", rc.get_launcher_cost(), "stall", _stall)
        if _stall > STALL_LIMIT:
            _dead = True
