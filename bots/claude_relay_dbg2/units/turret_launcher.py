...
def run():
    ...
    # Absorb slot 0 so a launcher shares the core's SYMMETRY consensus and its
    # position. A launcher never called comms.read(), so it ran on its own
    # untouched symmetry flags -- which diverge from the builder's by how much
    # each has seen -- and could disagree about where the enemy core is.
    try:
        comms.read()
    except Exception:
        pass
