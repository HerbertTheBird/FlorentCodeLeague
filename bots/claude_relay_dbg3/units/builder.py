...
def run():
    ...
    # #42 local debug: one line per builder-turn so a replay can be read directly.
    # Off on the ladder (log.DEBUG_LOGGING gates it); local oarena captures stdout.
    if DRAW_DEBUG or True:
        import relaygeom as _rg
        _p = map_info._my_pos
        try:
            _tc = _rg.their_core(); _d = _rg.dist_at((_p.x, _p.y)) if _tc else -1
        except Exception:
            _tc, _d = None, -1
        print("ST r=%-3d u=%-3d pos=(%2d,%2d) state=%-9s ti=%-4d scale=%-3d theircore=%s dist=%s" % (
