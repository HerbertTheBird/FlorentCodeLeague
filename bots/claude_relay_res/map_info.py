...
_siege_reserve: int = 0     # #42: price of the siege builder's NEXT hop


def set_siege_reserve(v: int) -> None:
    """Hold back one launcher's cost while the relay is still crossing.

    Measured on ragnarok: the relay threw at t2/t4/t6, then at t7 the bank was
    below the launcher price, relay.run() took its `waiting for money` branch,
    stalled out and stood down -- while OTHER builders spent the bank on barriers
    at t7 and t8. The chain died three hops short with the money going elsewhere.
    Tyr reserves exactly this and calls it the siege builder's next brick."""
    global _siege_reserve
    _siege_reserve = v
...
_ti_reserve_inner = ti_reserve_base


def ti_reserve() -> int:
    """Base reserve plus the siege builder's next hop (#42)."""
    return _ti_reserve_inner() + _siege_reserve
