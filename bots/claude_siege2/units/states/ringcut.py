...
MAX_SCORE = 9.7        # ABOVE heal(9.5)/route(8): a builder that crossed the
                     # map must not be dragged home to heal. Measured: the siege
                     # bot arrived, then spent 171 turns on heal and 81 on chase
                     # back at OUR core, and ringcut(7.4) never fired once.
ENGAGE_DIST = 8          # only once the relay has actually delivered us
USE_FLOW = True          # prefer carrying conveyors
LAUNCH_GUARD = False     # build a launcher to throw a guarding enemy bot away
...
def run(can_move=True):
    ...
    # ONCE CARDINALLY ADJACENT, STOP MOVING. Two reasons, both fatal:
    #  * you cannot attack the tile you are standing on, so stepping onto the
    #    target makes the attack impossible;
    #  * a conveyor is standable, so "move adjacent" will happily path THROUGH
    #    the target -- and vacating our square hands the enemy the adjacency it
    #    needs to heal what we were chipping (heal is Manhattan-1, 4 HP for 1 Ti
    #    against our 2 damage for 2 Ti).
    # Measured: bot 4 stepped left ONTO (3,4), the very tile it was attacking.
    # If we are adjacent and merely cannot afford the shot, WAIT -- do not wander.
