...
# #42 SIEGE WINDOW RESERVE. The relay dies when the bank drops below a launcher:
# best_hop asks can_build_launcher for its site, that is False when we cannot
# afford one, so "no hop" -> stall -> stand down. Measured on ragnarok: three
# throws, then at t7 the money was gone and OTHER builders spent it on 3 Ti
# barriers while the siege bot waited 12 turns and died three hops short.
# A per-unit reserve cannot fix that -- every builder runs its OWN interpreter and
# would each see _siege_reserve = 0. The ROUND NUMBER is the one thing they all
# agree on, so hold a launcher back for everyone during the crossing window.
SIEGE_WINDOW = 25
