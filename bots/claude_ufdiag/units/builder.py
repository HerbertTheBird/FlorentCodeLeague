...
def run():
    ...
    if rc.get_id() == 3 and rc.get_current_round() in (60,100,200,400,800):
        import units.states.unfreeze as _uf
        print("UFDIAG r=%d id=3 pos=%s best=%s ufscore=%s last=%s" % (
            rc.get_current_round(), (map_info._my_pos.x, map_info._my_pos.y),
            best_state.__name__.rsplit(".",1)[-1] if best_state else None,
            _uf.score(True), _uf._last.get(3)), flush=True)
