...
import units.states.unfreeze as unfreeze
...
states = tuple(sorted(
    [explore, disrupt, harvest, route, heal, attack, chase, chip, block, siege, unfreeze, unblock],
...
def run():
    ...
    if rc.get_id() == 3 and rc.get_current_round() in (200,400,800):
        _w = map_info._width
        _my = map_info._my_pos
        _pass = map_info.passable()
        _av = map_info.get_avoid(False)
        _out = []
        for _dx,_dy,_d in ((0,-1,Direction.NORTH),(1,0,Direction.EAST),(0,1,Direction.SOUTH),(-1,0,Direction.WEST)):
            _p = Position(_my.x+_dx, _my.y+_dy)
            if not map_info.in_bounds(_p): continue
            _n = _p.x + _p.y*_w
            _out.append("%s pass=%d avoid=%d blocked=%d can_move=%s" % (
                (_p.x,_p.y), (_pass>>_n)&1, (_av>>_n)&1,
                (map_info._bm_blocked>>_n)&1, rc.can_move(_d)))
        print("MVDIAG r=%d pos=%s | %s" % (rc.get_current_round(), (_my.x,_my.y), " || ".join(_out)), flush=True)
