...
from log import DRAW_DEBUG, log, dbg
...
def select_best_state(can_move=True, exclude=None):
    ...
    for state in states:
        ...
        _scored.append((state.__name__.split('.')[-1], round(float(score), 2)))
        if score > best_score:
            best_score = score
            best_state = state
    if can_move:
        try:
            dbg("STATE r=%d id=%d ti=%d chose=%s scores=%s" % (
                rc.get_current_round(), rc.get_id(), rc.get_global_resources(),
                best_state.__name__.split('.')[-1] if best_state else "NONE", _scored))
...
def run():
    ...
    try:
        from log import dbg
        _w = map_info._width
        _probe = [(3,14),(3,13),(3,15),(2,14),(4,14)]
        _out = []
        for (px,py) in _probe:
            _n = px + py*_w
            _out.append("%d%d%d%d%d" % (
                (map_info._bm_conveyors >> _n) & 1,
                (map_info._bm_team[map_info._my_team_idx] >> _n) & 1,
                (map_info._bm_route_targets >> _n) & 1,
                (map_info._bm_any_building >> _n) & 1,
                1 if rc.is_in_vision(Position(px,py)) else 0))
        _mc = map_info._my_core
        _cd = map_info.conv_dist_core
        _dd = []
        for (px,py) in [(3,13),(2,13),(3,14),(2,14),(1,12),(2,12)]:
            _n2 = px + py*_w
            _dir = map_info._building_dir[_n2] if _n2 < len(map_info._building_dir) else -1
            _dirname = {0:"N",1:"NE",2:"E",3:"SE",4:"S",5:"SW",6:"W",7:"NW",8:"C"}.get(_dir, str(_dir))
            _dd.append("(%d,%d)c%d d%s de%d f%s" % (
                px, py,
                (map_info._bm_conveyors >> _n2) & 1,
                (_cd[_n2] if _n2 < len(_cd) else "?"),
                (map_info._bm_dead_end >> _n2) & 1, _dirname))
        dbg("ROUTE r=%d id=%d ti=%d pos=(%d,%d) %s" % (
            current_round, rc.get_id(), rc.get_global_resources(),
            map_info._my_pos.x, map_info._my_pos.y, " ".join(_dd)))
        dbg("CONV r=%d id=%d ti=%d pos=(%d,%d) core=%s [conv,mine,routetgt,bld,vis]=%s" % (
            current_round, rc.get_id(), rc.get_global_resources(),
            map_info._my_pos.x, map_info._my_pos.y,
            (str(_mc.x)+","+str(_mc.y)) if _mc else "?", _out))
    except Exception as _e:
        dbg("CONVERR", _e)
