...
def _do_best_heal():
    ...
    try:
        from log import dbg
        _ca = map_info._bm_my_core_area
        _mp = map_info._my_pos
        _adj = bool(map_info.manhattan(_ca) & (1 << (_mp.x + _mp.y * map_info._width)))
        if _adj:
            _corehp = -1
            _m = _ca
            while _m:
                _b = _m & -_m; _n = _b.bit_length()-1; _m ^= _b
                _corehp = max(_corehp, map_info._building_hp[_n])
            dbg("HEALTRY r=%d id=%d ti=%d best=%s corehp=%d op=%d" % (
                rc.get_current_round(), rc.get_id(), rc.get_global_resources(),
                "yes" if best is not None else "NO", _corehp,
                1 if (rc.get_action_cooldown()==0 and rc.get_move_cooldown()==0) else 0))
    except Exception:
        pass
    if best is not None:
        rc.heal(best)
