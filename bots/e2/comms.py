...
# Opening conveyor-plan override of slot 0. On the turn the core spawns a
# starting-conv builder it replaces its normal word with a plan word, told apart
# by the top bit (bit 31). A normal core word never sets bit 31 -- the tally is
# capped one bit short for exactly that (see `_TALLY_MASK` in init). The whole
# low 31 bits carry the DFS stream (3 bits/conveyor, so up to 10 conveyors).
# No root position or excluded side is sent: the core writes a plan word only on
# a turn it spawns exactly one plan-builder, so a builder seeing the marker on
# its first turn IS that builder and its own tile IS the root; the core-ward
# (excluded) side is the root's single core-adjacent neighbour, derived by the
# reader. See conveyor_plan.py for the DFS body.
_PLAN_MARKER = 1 << 31
...
_pending_core_plan = None    # core: conveyor-plan payload to write into slot 0 this turn (or None)
...
_core_distress = 0           # latest core_vitals verdict read from the core slot
_core_pos = None             # our core's tile, latched from slot 0 (see core_position)
...
def init(c: Controller):
    ...
    global _TALLY_MASK, _WIRE_TO_ENV, _DISTRESS_SHIFT
    global _my_slot, _my_prev_pos, _my_hb, _last_written, _probe_off, _probe_step
    global _pending_path_done, _pending_core_plan, _route_total, _core_route_total
    global _core_distress
    global _core_pos
    ...
    # minus the top bit which is reserved as the conveyor-plan marker).
    _SYM_SHIFT = _POS_BITS + 2
    # Two bits of core distress (core_vitals.FINE/ENGAGED/CRITICAL), taken off
    # the route tally -- which had 16 bits for a number that counts connected
    # routes and never approaches 14 bits' worth.
    _DISTRESS_SHIFT = _POS_BITS + 5
    _TALLY_SHIFT = _POS_BITS + 7
    _TALLY_MASK = (1 << (31 - _TALLY_SHIFT)) - 1
...
def core_position():
    """Our core's own tile, decoded from its slot-0 broadcast.

    A relay launcher thrown halfway across the map has never had the core in
    vision, so `map_info._my_core` is None in its interpreter and the enemy core
    cannot be reflected out of it. The core publishes its position every round;
    this is the only way to recover it. Latched, because on a turn slot 0
    carries a conveyor plan instead there is no position in the word.
    """
    global _core_pos
    val = rc.read_store(_CORE_SLOT)
    if val and not (val & _PLAN_MARKER) and ((val >> _VALID_SHIFT) & 1):
        n = val & _POS_MASK
        _core_pos = Position(n % _width, n // _width)
    return _core_pos
...
def core_distress() -> int:
    """The core's own verdict on its health -- core_vitals.FINE / ENGAGED /
    CRITICAL. Read from slot 0, so it is current everywhere on the map, not
    just where somebody can see the core.

    A conveyor-plan word (bit 31) carries no verdict; the last one read stands,
    which is right -- the plan word is written on a single opening turn.
    """
    return core_vitals.evaluate() if _am_core else _core_distress
...
# --------------------------------------------------------------------------- #
# Opening conveyor plan (slot-0 override; see the module header + conveyor_plan)
# --------------------------------------------------------------------------- #
def core_plan_dfs_budget() -> int:
    """Bits available for the DFS stream in one plan word: everything below the
    marker (bit 31), i.e. 31 bits -> up to 10 conveyors."""
    return 31


def queue_core_plan(dfs_bits) -> None:
    """Core, before write(): queue a conveyor plan to publish in slot 0 THIS turn
    instead of the normal core word. The freshly spawned builder reads it next
    turn (buffered writes) via read_core_plan(). Excess bits beyond 31 are
    dropped -- the caller should have pruned to core_plan_dfs_budget()."""
    global _pending_core_plan
    payload = 0
    for i, b in enumerate(dfs_bits):
        if i >= 31:
            break                                  # never touch the marker bit
        if b:
            payload |= 1 << i
    _pending_core_plan = payload & (_PLAN_MARKER - 1)


def read_core_plan():
    """The DFS bitstream (list[int]) if slot 0 currently holds a plan word, else
    None. Root and excluded side are not carried -- the reader is the builder at
    the tree root, so it supplies both itself. Reuses this round's cached raw
    read when available."""
    val = _slot_raw[_CORE_SLOT] if _raw_round == rc.get_current_round() \
        else rc.read_store(_CORE_SLOT)
    if not (val & _PLAN_MARKER):
        return None
    return [(val >> i) & 1 for i in range(31)]
...
# Encode / broadcast
# --------------------------------------------------------------------------- #
def _encode_core(pos: Position, hb: int, sym_bits: int, distress: int,
                 tally: int) -> int:
    val = (pos.x + pos.y * _width) & _POS_MASK
    val |= (hb & 1) << _HB_SHIFT
    val |= 1 << _VALID_SHIFT
    val |= (sym_bits & 0x7) << _SYM_SHIFT
    val |= (distress & 0x3) << _DISTRESS_SHIFT
...
def _broadcast():
    global _my_prev_pos, _my_hb, _last_written, _pending_path_done, _pending_core_plan
    my_pos = map_info._my_pos
    if _am_core:
        if _pending_core_plan is not None:
            # Override slot 0 with the conveyor plan this turn. Still flip the
            # heartbeat so the word differs from both the previous and the next
            # normal core word (readers' liveness test is "raw value changed").
            val = _PLAN_MARKER | _pending_core_plan
            ...
            _pending_core_plan = None
            return
        val = _encode_core(my_pos, _my_hb, _my_sym_bits(),
                           core_vitals.evaluate(), _route_total)
    else:
...
def _absorb():
    ...
    for s in range(_STORE_SIZE):
        ...
        if s == _CORE_SLOT:
            if val & _PLAN_MARKER:
                # A conveyor-plan word this turn, not a core word. Only the
                # freshly spawned builder wants it (read_core_plan); everyone
                # else must NOT misdecode it as position/sym/tally. Leave the
                # last known sym/tally untouched and skip the baseline update.
                continue
            sym_bits = (val >> _SYM_SHIFT) & 0x7
            map_info.update_symmetry_from_comms(sym_bits)
            _core_route_total = (val >> _TALLY_SHIFT) & _TALLY_MASK
            _core_distress = (val >> _DISTRESS_SHIFT) & 0x3
