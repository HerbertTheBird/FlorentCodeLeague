...
states = tuple(sorted(
    [explore, disrupt, harvest, route, heal, attack, chase, chip, block, siege, relay, ringcut, unblock],
...
def select_best_state(can_move=True, exclude=None):
    ...
    best_score = 0
    _scored = []
    _banned = _SIEGE_BANNED if _is_eternal_sieger() else ()
    for state in states:
        if state is exclude:
            continue
        if _banned and state.__name__.rsplit(".", 1)[-1] in _banned:
            continue
...
def run():
    ...
    pass
    # The eternal-sieger ban can forbid every state that has work, in which case
    # select_best_state() returns None. Calling .run() on it raised
    # "'NoneType' object has no attribute 'run'" -- measured 2698 times in a single
    # 8-game run, swallowed by main.py's except, so the unit silently did nothing.
...
# --- #42b ETERNAL SIEGER -------------------------------------------------------
# The siege bot crossed the map and then walked home: its state histogram after
# arrival was heal 171, chase 81, route 41, all of it back at OUR core. It did not
# need a new high-priority state -- it needed to be FORBIDDEN the states that drag
# it home. So the whole rule is: once the designated siege bot has arrived, it may
# not harvest, may not route, and may not chase. What remains -- ringcut, siege,
# chip, block, attack, heal -- is all enemy-side work, so it stays where it is.
_SIEGE_BANNED = ("harvest", "route", "chase", "explore", "heal")


def _is_eternal_sieger() -> bool:
    try:
        import relaygeom
        if rc.get_id() not in relaygeom.RELAY_BOT_IDS:
            return False
        tc = relaygeom.their_core()
        if tc is None:
            return False
        me = map_info._my_pos
        # "Inside enemy territory" -- close enough that going home is never right.
        return max(abs(me.x - tc[0]), abs(me.y - tc[1])) <= ETERNAL_DIST
    except Exception:
        return False
