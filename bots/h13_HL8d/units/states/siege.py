...
MIN_PROGRESS = 0.25
# Never target a ring further than this, at ANY completion level.
SIEGE_HARD_LEASH = 8
# Extra-tight bound for a ring nothing has been placed on yet.
SIEGE_SOFT_LEASH = 6
...
def score(can_move=True):
    ...
    prog = _progress()
    # HARD LEASH, applied on BOTH branches. The previous version only leashed the
    # untouched-ring branch; once a ring was 25% sealed the score had no distance
    # term at all, so a builder anywhere on the map could be dragged to it -- and
    # the score ROSE as the ring filled. That is exactly the failure mode recorded
    # for the two earlier barrier attempts ("preferring the ring OVERRODE distance
    # ordering and sent builders across the map": core-ring barriering +0.0012,
    # disrupt-at-6 -4.6).
    if dist > SIEGE_HARD_LEASH:
        return 0
    if prog < MIN_PROGRESS:
        # Untouched ring: needs us to be closer still.
        return MAX_SCORE * 0.5 if dist <= SIEGE_SOFT_LEASH else 0
    # Rises toward MAX_SCORE as the ring nears completion, decayed by distance so a near builder always outranks a far one.
    return MAX_SCORE * (0.5 + 0.5 * prog) * (1.0 - 0.5 * dist / SIEGE_HARD_LEASH)
