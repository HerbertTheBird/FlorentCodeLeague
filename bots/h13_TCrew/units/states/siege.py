...
def score(can_move=True):
    ...
    if rc.get_global_resources() < rc.get_barrier_cost() + map_info.ti_reserve():
        return 0
    # TRIGGER: only with spare hands. A siege costs builder-TURNS, which is the
    # scarce resource (an L-conveyor chain already costs ~2L builder-rounds), so
    # do not start one on a small crew.
    if map_info._bm_friendly_bots.bit_count() < SIEGE_MIN_CREW:
        return 0
    ...
    if prog < MIN_PROGRESS:
        ...
        return MAX_SCORE * 0.5 if dist <= 6 else 0
