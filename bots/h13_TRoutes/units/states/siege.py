...
def score(can_move=True):
    ...
    if rc.get_global_resources() < rc.get_barrier_cost() + map_info.ti_reserve():
        return 0
    # TRIGGER: our own economy must be running before we spend builder-turns on
    # theirs. attack.score() already gates enemy-core targeting the same way.
    if comms.route_total() < SIEGE_MIN_ROUTES:
        return 0
