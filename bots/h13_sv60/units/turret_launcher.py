...
# ---- #33 offensive forward-throw ---------------------------------------------
TREK_THROW = True


def _try_throw_trekker() -> bool:
    """Throw an adjacent friendly builder toward the ENEMY core.

    Deliberately skipped for the sentry launcher, whose whole job is defensive
    interception near our own core -- forward-throwing our defender would be an
    own goal. Any OTHER launcher is one a trekker built for exactly this.
    """
    if not TREK_THROW:
        return False
    ...
    me = map_info._my_pos
    try:
        sentry = _def.sentry_launcher_pos()
        if sentry is not None and sentry.x == me.x and sentry.y == me.y:
            return False
    except Exception:
        pass
    w = map_info._width
    ...
    bit = 1 << (me.x + me.y * w)
    near = map_info.manhattan(bit, 2) & ~bit
    friends = near & map_info._bm_friendly_bots & ~mine if False else near & map_info._bm_friendly_bots
    if not friends:
        return False
    cn = (core & -core).bit_length() - 1
    cx, cy = cn % w, cn // w
    ...
    b = friends & -friends
    n = b.bit_length() - 1
    bot_pos = Position(n % w, n // w)
    cur = d2core(bot_pos.x, bot_pos.y)
    best = None
    best_d = cur
    ...
    for dy in range(-5, 6):
        for dx in range(-5, 6):
            dd = dx * dx + dy * dy
            if dd < 1 or dd > 26:                 # launcher throw disk
                continue
            tx, ty = me.x + dx, me.y + dy
            if not (0 <= tx < w and 0 <= ty < map_info._height):
                continue
            if not ((walk >> (tx + ty * w)) & 1):
                continue
            d = d2core(tx, ty)
            if d < best_d:
                best_d = d
    ...
    if best is None:
        return False
    if not rc.can_launch(bot_pos, best):
        return False
    rc.launch(bot_pos, best)
    log(f"launcher forward-threw trekker {bot_pos} -> {best} (d {cur} -> {best_d})")
    return True
