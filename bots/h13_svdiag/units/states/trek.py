...
_TD=[0,0,0,0,0,0]   # run, claimed, adj_launcher, too_poor, cbl_false, BUILT
...
def td_report():
    ...
    print("TREKDIAG run=%d reached_cost=%d adj_launcher=%d too_poor=%d tried_build=%d BUILT=%d bank=%d cost=%d reserve=%d"
          % (_TD[0],_TD[1],_TD[2],_TD[3],_TD[4],_TD[5],
             rc.get_global_resources(), rc.get_launcher_cost(), _mi.ti_reserve()), flush=True)
