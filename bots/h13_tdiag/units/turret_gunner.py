...
def _fire_probe(rc, tgt):
    """#34 instrumentation: how many of our shots land on a target the enemy can
    heal? Heal is 4 HP/Ti against a gunner's 1.75 dmg/Ti, so a shot into a target
    with >=2 adjacent healers is titanium burnt for nothing."""
    import map_info as _mi
    try:
        w = _mi._width
        bit = 1 << (tgt.x + tgt.y * w)
        adj = _mi.manhattan(bit) & ~bit
        healers = (adj & _mi._bm_enemy_bots).bit_count()
        hp = _mi._building_hp[tgt.x + tgt.y * w]
        print("FIREPROBE healers=%d hp=%d" % (healers, hp), flush=True)
    except Exception as _e:
        print("FIREPROBE err", flush=True)
