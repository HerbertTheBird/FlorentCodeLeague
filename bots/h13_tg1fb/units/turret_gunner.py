...
def run():
    ...
    if fire_target is not None and rc.can_fire(fire_target):
        if _heal_contested(rc, fire_target, GameConstants.GUNNER_DAMAGE):
            return          # hold ammo; must NOT fall through to scrap/rotate
        rc.fire(fire_target)
...
HEAL_BLOCK = 1      # refuse when adjacent enemy bots >= this
FINISH_MULT = 2.0
