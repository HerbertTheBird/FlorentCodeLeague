...
SIEGE_BOTS = 1
# Only the first few rounds can claim the role -- this is an OPENING commitment.
CLAIM_BEFORE_ROUND = 4
...
def score(can_move=True):
    ...
    if uid not in _trekkers:
        if (rc.get_current_round() <= CLAIM_BEFORE_ROUND
...
def run(can_move=True):
    ...
    if hops < HOPS_MAX:
        ...
        if rc.get_global_resources() >= cost + map_info.ti_reserve():
            my = map_info._my_pos
