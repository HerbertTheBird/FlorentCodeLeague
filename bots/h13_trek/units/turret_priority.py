...
from fcode import EntityType, Position, GameConstants
import map_info
...
def scrap_if_idle(rc) -> bool:
    ...
    if map_info._bm_enemy_bots:
        return False
    # AREA CONTROL: never scrap a turret that covers either core ring.
    #
    # The 8 cardinal tiles around a core are the only ones that can deliver into
    # it (delivery is a directed cardinal push; diagonals carry nothing), so they
    # are the highest-value ground on the board. A turret covering OUR ring
    # defends the sink our whole economy passes through; one covering THEIRS
    # holds the siege ground. Neither stops being useful merely because no enemy
    # bot is visible this turn -- which is all the old guard checked.
    #
    # "Covers" = within this turret's own squared radius of the core footprint
    # (SENTINEL 32, GUNNER 13). The gunner test is deliberately omnidirectional:
    # it can rotate, so where its ray points today does not decide whether the
    # POSITION is worth holding.
    #
    # Measured, 24 games vs herbert12: +0.0267 (the documented "<= 2 enemy
    # buildings" guard measured -0.0081; never scrapping at all measured +0.2767
    # on a self-play screen but only +0.0345 on 86 games, and split by opponent:
    # +0.128 vs Tyr_Jython, -0.083 vs V6_earlysiege). Adopted as the middle
    # ground: keeps the scale refund where it is genuinely free, holds the ground
    # that matters.
    my = map_info._my_pos
    et_idx = map_info._building_et_idx[my.x + my.y * map_info._width]
    if et_idx == map_info._IDX_SENTINEL:
        r2 = GameConstants.SENTINEL_VISION_RADIUS_SQ
    else:
        r2 = GameConstants.GUNNER_VISION_RADIUS_SQ
    w = map_info._width
    m = map_info._bm_my_core_area | map_info._bm_their_core_area
    while m:
        b = m & -m
        n = b.bit_length() - 1
        m ^= b
        ...
        dy = (n // w) - my.y
        if dx * dx + dy * dy <= r2:
            return False                  # holds core-ring ground: keep it
    rc.self_destruct()
    return True
