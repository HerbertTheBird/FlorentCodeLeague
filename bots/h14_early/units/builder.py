...
states = tuple(sorted(
    [explore, disrupt, harvest, route, heal, attack, chase, chip, block, siege, unblock],
...
def run():
    ...
    if rc.get_current_round() <= 60 and rc.get_current_round() % 4 == 0:
        try:
            import map_info as _mi
            _my = _mi._my_pos
            _ok = 0
            for _d in ((1,0),(-1,0),(0,1),(0,-1)):
                _p = Position(_my.x+_d[0], _my.y+_d[1])
                if _mi.in_bounds(_p) and rc.can_build_launcher(_p, Direction.NORTH):
                    _ok += 1
            print("EARLY r=%d bank=%d launcher_cost=%d conveyor=%d reserve=%d can_build_dirs=%d" % (
                rc.get_current_round(), rc.get_global_resources(), rc.get_launcher_cost(),
                rc.get_conveyor_cost(), _mi.ti_reserve(), _ok), flush=True)
        except Exception as _e:
            print("EARLY err %s" % _e, flush=True)
    comms.write()
