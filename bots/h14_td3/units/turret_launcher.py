...
def run():
    ...
    print('LRUN r=%d id=%d' % (rc.get_current_round(), rc.get_id()), flush=True)
    map_info.update()
    _classify_launcher()
...
def _try_relay_throw() -> bool:
    ...
    try:
        ...
        pass
    _r = rc.get_current_round()
    _adj = _adjacent_friendly_builders_with_ids()
    if _r < 30:
        import map_info as _mi
        print("TD3 r=%d me=(%d,%d) adj=%d core=%d ring=%d bands=%d hs=%d vs=%d rs=%d solved=%d acd=%s" % (
            _r, map_info._my_pos.x, map_info._my_pos.y, len(_adj),
            relay.their_core_area().bit_count(), relay._ring().bit_count(),
            len(relay._bands()), _mi._hor_sym, _mi._ver_sym, _mi._rot_sym, _mi._solved_sym,
            rc.get_action_cooldown()), flush=True)
    for uid, bot_pos in _adj:
        if not relay.is_siege_id(uid):
            continue        # only a siege builder rides the relay
        cur = relay.dist_at(bot_pos.x, bot_pos.y)
        if _r < 30: print("TD3b r=%d uid=%s bot=(%d,%d) cur=%d siege=%s" % (_r, uid, bot_pos.x, bot_pos.y, cur, relay.is_siege_id(uid)), flush=True)
        if cur >= 999:
            continue
        ...
        if landing is None:
            if _r < 30: print("TD3c r=%d NO_LANDING cur=%d" % (_r, cur), flush=True)
            continue
        _cl = rc.can_launch(bot_pos, landing)
        if _r < 30: print("TD3d r=%d land=(%d,%d) can_launch=%s" % (_r, landing.x, landing.y, _cl), flush=True)
        if _cl:
