...
def _try_relay_throw() -> bool:
    ...
    if not relay.siege_ids:
        if _r < 60: print("THROWDIAG r=%d no_siege_ids" % _r, flush=True)
        return False
    me = map_info._my_pos
    try:
    ...
    if _r < 60:
        print("THROWDIAG r=%d ids=%s adj=%s" % (_r, sorted(relay.siege_ids), [(a,(b.x,b.y)) for a,b in _adj]), flush=True)
    for uid, bot_pos in _adj:
        if uid not in relay.siege_ids:
            continue
        hop = relay.best_hop(bot_pos.x, bot_pos.y)
        if hop is None:
            if _r < 60: print("THROWDIAG r=%d uid=%d no_hop" % (_r, uid), flush=True)
            continue
        _site, landing = hop
        _cl = rc.can_launch(bot_pos, landing)
        if _r < 60: print("THROWDIAG r=%d uid=%d land=(%d,%d) can_launch=%s acd=%s" % (_r, uid, landing.x, landing.y, _cl, rc.get_action_cooldown()), flush=True)
        if _cl:
