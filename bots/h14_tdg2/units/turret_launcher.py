...
def _try_relay_throw() -> bool:
    ...
    if _r < 80:
        print("TD r=%d me=(%d,%d) adj=%d acd=%s" % (_r, me.x, me.y, len(_adj), rc.get_action_cooldown()), flush=True)
    for uid, bot_pos in _adj:
        cur = relay.dist_at(bot_pos.x, bot_pos.y)
        if _r < 80:
            print("TD2 r=%d bot=(%d,%d) cur=%d ring=%d core=%d bands=%d" % (
                _r, bot_pos.x, bot_pos.y, cur, relay._ring().bit_count(),
                relay.their_core_area().bit_count(), len(relay._bands())), flush=True)
        if cur >= 999:
            continue
        ...
        if landing is None:
            if _r < 80: print("TD r=%d uid=%s cur=%d NO_LANDING" % (_r, uid, cur), flush=True)
            continue
        _cl = rc.can_launch(bot_pos, landing)
        if _r < 80: print("TD r=%d uid=%s bot=(%d,%d) land=(%d,%d) can_launch=%s" % (_r, uid, bot_pos.x, bot_pos.y, landing.x, landing.y, _cl), flush=True)
        if _cl:
