...
def run():
    ...
    if rc.get_id() == 3 and rc.get_current_round() in (200, 400):
        _mi = map_info
        _w = _mi._width
        for _tx, _ty in ((9, 6), (7, 6), (8, 7)):
            _n = _tx + _ty * _w
            _et = _mi._building_et_idx[_n]
            _names = {_mi._IDX_CORE:"core", _mi._IDX_CONVEYOR:"conveyor", _mi._IDX_HARVESTER:"harvester",
            ...
            _in = []
            for _nm, _m in (("WALL", _mi._bm_env[_mi._IDX_ENV_WALL]), ("harv", _mi._bm_et[_mi._IDX_HARVESTER]),
                            ("gun", _mi._bm_et[_mi._IDX_GUNNER]), ("sent", _mi._bm_et[_mi._IDX_SENTINEL]),
                            ("laun", _mi._bm_et[_mi._IDX_LAUNCHER]), ("barr", _mi._bm_et[_mi._IDX_BARRIER]),
                            ("conv", _mi._bm_et[_mi._IDX_CONVEYOR]), ("mycore", _mi._bm_my_core_area),
                            ("theircore", _mi._bm_their_core_area), ("seen", _mi._bm_seen)):
                if (_m >> _n) & 1: _in.append(_nm)
            print("MASK r=%d (%d,%d) et_idx=%s(%s) blocked=%d pass=%d in=[%s]" % (
                rc.get_current_round(), _tx, _ty, _et, _names.get(_et, "?"),
                (_mi._bm_blocked >> _n) & 1, (_mi.passable() >> _n) & 1, ",".join(_in)), flush=True)
