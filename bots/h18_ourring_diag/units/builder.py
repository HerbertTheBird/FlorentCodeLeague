...
def run():
    ...
    if rc.get_current_round() % 200 == 0:
        our_ring_report()
    comms.write()


# ---- #50 instrumentation: is anything enemy ever ON OUR core ring? -------------
_OR = [0, 0, 0]   # builder-turns sampled, turns with an enemy structure on our ring, guarded


def our_ring_probe():
    import map_info as _mi
    _OR[0] += 1
    core = _mi._bm_my_core_area
    if not core:
        return
    w, h = _mi._width, _mi._height
    ring = 0
    m = core
    while m:
        b = m & -m; n = b.bit_length() - 1; m ^= b
        x, y = n % w, n // w
        for dx in (-1, 0, 1):
            for dy in (-1, 0, 1):
                if 0 <= x+dx < w and 0 <= y+dy < h:
                    ring |= 1 << ((x+dx) + (y+dy)*w)
    ring &= ~core & _mi._board_mask
    theirs = _mi._bm_team[1 - _mi._my_team_idx]
    hostile = ring & _mi._bm_any_building & theirs
    if hostile:
        _OR[1] += 1
        # guarded = an enemy bot adjacent to it (heals 4 HP/Ti vs our 2 dmg/2 Ti)
        gm = hostile
        while gm:
            b = gm & -gm; gm ^= b
            if _mi.manhattan(b) & ~b & _mi._bm_enemy_bots:
                _OR[2] += 1
                break


def our_ring_report():
    print("OURRING uid=%d sampled=%d hostile_on_ring=%d guarded=%d" % ((rc.get_id(),)+tuple(_OR)), flush=True)
