...
def score(can_move=True):
    ...
    if open_ring and rc.get_global_resources() >= rc.get_barrier_cost():
        ...
        if t is not None:
            _target = t
            return MAX_SCORE
    # Nothing to build: STAY. Never hand this bot back to heal/route/chase, which
    # is what walked it home across the map.
    _walk_to = None if d <= 3 else nav.closest(ring, to_adjacent=True)[0]
    return MAX_SCORE
