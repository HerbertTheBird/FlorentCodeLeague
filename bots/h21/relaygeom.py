...
def best_hop(stand, site_ok=None):
    ...
    """
    cur = dist_at(stand)
    # #47 ESCAPE. A throw is a wall-ignoring teleport, so landing inside a sealed
    # pocket is harmless -- we simply throw again. What was fatal is bailing here:
    # once the bot is inside the chamber and can SEE its walls, the flood from the
    # enemy ring can no longer reach it, dist_at goes infinite, and best_hop
    # returned None on this very line -- so it never even considered launching
    # out. _stall then hit STALL_LIMIT, the relay marked itself dead, and the FIRST
    # builder of the game oscillated two tiles for the remaining 117 turns
    # (royale, match e376aac3, bot 3 thrown into the middle chamber at t2).
    # When our own distance is unknown, ANY landing with a finite distance is
    # progress, so keep launchering instead of giving up.
    escaping = cur >= (1 << 20)
    if escaping:
        cur = (1 << 30)
    best = None
