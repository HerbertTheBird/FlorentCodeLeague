...
# Unscaled titanium floor every build must leave behind, so heal/attack actions
# stay affordable.
# An UNSCALED floor is held at all times so a builder can always afford its
# per-turn action. Heal costs BUILDER_BOT_HEAL_COST=1 for HEAL_AMOUNT=4 HP --
# 4 HP per titanium, four times attack's 2 damage for 2 Ti, and the most
# efficient use of titanium in the game. Action costs do NOT scale with build
# scale, so the floor does not either.
#
# Measured (herbert9, vase/antler/ragnarok, both sides): of 86 core-adjacent
# builder-turns with a DAMAGED core, 23 produced no heal -- and 22 of those 23
# had ZERO titanium. Median bank during core fights was 3 Ti. So 96% of missed
# core heals were pure bankruptcy, not targeting or cooldown.
ACTION_FLOOR = 5
