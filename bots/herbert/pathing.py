...
class Pathing:
    ...
    def bfs_move(self, start_n: int, target_mask: int, avoid: int, avoid_turret: bool = True):
        ...
        # If we are STANDING in enemy threat, force a step out: drop "stay put"
        # as an option so the search must move. Entering threat is still allowed
        # (it just costs +tcost below), so we can cross a line of threat without
        # ever resting in it. Keep "stay" only when boxed in (no legal move).
        start_in_threat = bool(threat & start_mask)
        threat &= ~start_mask
