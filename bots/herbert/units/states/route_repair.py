...
def _my_claims():
    ...
    # route_repair: only a candidate that can be connected to the accepting
    # network in a single hop counts -- i.e. one cardinally adjacent to a valid
    # route target (a conveyor whose chain already reaches the core, or the core
    # itself). These "one hop from done" repairs run above harvest_repair/route.
