...
def _get_attack_candidates():
    ...
    if not can_afford_sent and not can_afford_gun:
        metrics.rej("attack", "cannot_afford_turret",
    ...
    if not filtered:
        metrics.rej("attack", "no_placement_tile")
        return 0
    ...
    if max_score < MIN_ATTACK_SCORE + THREAT_PENALTY:
        metrics.rej("attack", "below_min_score", best=int(max_score),
                    need=int(MIN_ATTACK_SCORE + THREAT_PENALTY))
        return 0
...
def score(can_move=True):
    ...
    if metrics.ENABLED and rc.get_current_round() % 50 == 0:
        metrics.act("siege", "gate", routes=comms.route_total(),
                    open=(1 if comms.route_total() >= 2 else 0))
