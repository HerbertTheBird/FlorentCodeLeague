...
def run():
    ...
    # LOCAL DEBUG: one line per BUILDER per TURN. oarena captures bot stdout into
    # the replay, so `python3 tools/replay_state.py <replay> --meta` or a stdout
    # scan shows exactly which state each unit chose and why it looked that way.
    # This build is debug-only -- the ladder bot (herbert18) has no prints.
    try:
