...
def possible_ore():
    ...
    return (ore
            ...
            & ~friendly_blocking)
    # Ore under enemy turret coverage is no longer excluded outright. This
    # measured EXACTLY inert in self-play (0 of 86 maps changed), because locally
    # the ore that gets covered already has a harvester on it. The ladder shows
    # the case self-play cannot: on fjordgate (10x10) the opponent fielded 3
    # gunners and 4 sentinels, whose coverage on a 100-tile board reaches most of
    # the ore, and we finished a 386-turn game having built ONE harvester (to
    # their three) while laying 18 conveyors to reach it. A harvester is 30 HP and
    # repays its cost in about eight rounds; refusing to build one under any
    # coverage at all is the wrong trade on a small map.
def harvestable_ore():
    ore = possible_ore()
