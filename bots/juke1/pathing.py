...
class Pathing:
    ...
    def bfs_move(self, start_n: int, target_mask: int, avoid: int, avoid_turret: bool = True):
        ...
        # ---- Gunner juke ---------------------------------------------------
        # Standing in a GUNNER's line of fire (specifically a gunner -- its ray is
        # fixed and rotating costs it) while already king-adjacent (Chebyshev-1) to
        # the target: don't flee. Step to the nearest reachable tile that is STILL
        # king-adjacent to the target but OUT of the gunner's threat, so we dodge
        # the shot yet hold position to resume acting. Forced move -- staying just
        # eats the shot. If no such tile is reachable, fall through to the ordinary
        # search (regular bfs move).
        gunner = map_info._bm_enemy_gunner_threat
        if start_mask & gunner:
            cheb_t = map_info.expand_chebyshev(target_mask)
            if start_mask & cheb_t:
                stay_set = cheb_t & ~gunner & not_avoid & ~builders_mask & ~start_mask
                if stay_set:
                    dodge = self._dodge_step(start_n, stay_set, not_avoid, builders_mask)
    ...
    def _dodge_step(self, start_n, goal_mask, not_avoid, builders_mask):
        """Gunner-juke move: from start_n pick the cardinal first step that most
        quickly reaches a tile in `goal_mask` (king-adjacent to the target AND out
        of gunner threat), flooding over `not_avoid`. A first step that lands
        directly in goal_mask wins (distance 0). Returns (start_pos, step_pos,
        dist), or None if no legal first step can reach goal_mask."""
        w = self.width
        start_mask = 1 << start_n
        nlc = map_info._not_left_col
        nrc = map_info._not_right_col
        step_ring = (map_info.expand_manhattan(start_mask)
                     & not_avoid & ~builders_mask & ~start_mask)
        if not step_ring:
            return None
        best_step = None
        best_dist = None
        ...
        while sr:
            ...
            sr ^= b
            frontier = b
            visited = start_mask | b
            d = 0
            while frontier:
                if frontier & goal_mask:
                    break
                d += 1
                frontier = (((frontier & nrc) << 1) | ((frontier & nlc) >> 1)
                            | (frontier << w) | (frontier >> w)) & not_avoid & ~visited
                visited |= frontier
            else:
                continue                       # never reached goal from this step
            if best_dist is None or d < best_dist:
                best_dist = d
                bn = b.bit_length() - 1
                best_step = Position(bn % w, bn // w)
        if best_step is None:
            return None
        return Position(start_n % w, start_n // w), best_step, best_dist
