...
# LAUNCHER SIEGE DISABLED (user request). The launcher relay is the transport
# that ferries a designated builder to the enemy core by building a chain of
# launchers and throwing itself forward. Turning this off means:
#   * no launcher is ever built for transport, so the SIEGE_WINDOW reserve below
#     is also switched off (map_info.SIEGE_WINDOW = -1),
#   * `relaygeom.RELAY_BOT_IDS` still names bots 3 and 4, but they never travel,
#     so `_is_eternal_sieger()` never engages (it needs ETERNAL_DIST of their
#     core) and the harvest/route/chase/explore ban never applies,
#   * ringcut/ringhold remain compiled in but only engage if a builder reaches
#     the enemy ring under its own power, which is the pre-siege behaviour.
