...
def score(can_move=True):
    global _cached
    _cached = None
    # #45: an enemy barrier on ORE is worth breaking on its own merits -- it is a
    # harvester site being denied, independent of whether any route is unreachable.
    _ore = barriered_ore()
    if _ore:
        _ore &= ~map_info._bm_enemy_turret_threat
        if _ore and rc.get_global_resources() >= GameConstants.BUILDER_BOT_ATTACK_COST:
            t, d = nav.closest(_ore, to_adjacent=True)
...
# ---- #45 barriered ore --------------------------------------------------------
# An enemy barrier sitting ON an ore tile denies us a HARVESTER SITE, not just a
# path. A barrier is BARRIER_MAX_HP 30 = 15 builder-turns / 30 Ti; a harvester
# yields 2.5 Ti/round forever, so it repays in ~12 rounds and then compounds.
# Worth far more than the generic "break a blocker" case, and unlike the rejected
# conveyor re-weighting (#41) the thing we buy is not 3 Ti of rebuildable belt.
# We already know ore denial matters: turning OUR ore-barriering off measured
# -0.0793, so the enemy doing it to us costs us symmetrically.
ORE_SCORE = 7.2


def barriered_ore():
    """Enemy barriers standing on ore tiles we could otherwise harvest."""
    theirs = map_info._bm_team[1 - map_info._my_team_idx]
    ore = map_info._bm_env[map_info._IDX_ENV_ORE_TI]
    return map_info._bm_et[map_info._IDX_BARRIER] & theirs & ore
