...
# --- Barriered ore ON A LIVE LINE ----------------------------------------
# #45, gated version. An enemy building sitting on ore denies us a harvester
# site, but clearing ore we have no belt to is a bad trade on its own: the
# building is 30 HP and a builder does 2 dmg per 2 Ti, so 30 Ti and 15 turns buy
# a bare ore tile. Measured that way (ore1, 50 games) it is -0.2218.
#
# The trade only pays where the ore is ALREADY adjacent to one of my conveyors
# whose chain reaches my core -- `_bm_reaches_core`. Then the barrier is the
# single thing between us and a harvester that delivers 2.5 Ti/round from the
# turn it goes up, with no belt left to build. That is the case the task asks
# for: "especially where a route already runs from it".
ORE_SCORE = 7.2
...
def _live_line_ore() -> int:
    """Enemy breakables on ore that is orthogonally adjacent to a live belt of mine."""
    global _ORE_ROUND, _ORE_VAL
    ...
    blocked_ore = (map_info._bm_blocked & ~mine
                   & ~map_info._bm_their_core_area
                   & ~map_info._bm_env[map_info._IDX_ENV_WALL]
                   & ore)
    if not blocked_ore:
        return 0
    live = map_info._bm_reaches_core
    if not live:
        return 0
    # A harvester outputs to an orthogonally adjacent belt tile, so the ore has to
    # touch the live belt for breaking the barrier to finish the job by itself.
    _ORE_VAL = blocked_ore & map_info.manhattan(live)
    return _ORE_VAL
