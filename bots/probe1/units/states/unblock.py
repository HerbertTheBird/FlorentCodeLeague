...
def _probe():
    """#45 funnel: how much barriered ore exists, and how much of it is usable."""
    global _PROBE_ROUND
    r = rc.get_current_round()
    if r == _PROBE_ROUND:
        return
    ...
    blocked = (map_info._bm_blocked & ~mine
               & ~map_info._bm_their_core_area
               & ~map_info._bm_env[map_info._IDX_ENV_WALL] & ore)
    if not blocked:
        return
    my_convs = map_info._bm_conveyors & mine
    live = map_info._bm_reaches_core
    adj_any = blocked & map_info.manhattan(my_convs) if my_convs else 0
    adj_live = blocked & map_info.manhattan(live) if live else 0
    metrics.rej("probe45", "blocked_ore",
                n=bin(blocked).count("1"),
                any=bin(adj_any).count("1"),
                live=bin(adj_live).count("1"))
