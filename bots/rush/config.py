...
SLOT_HEALER_ID = 5               # the home healer, once one exists

# The late healer.
#
# Healing is 1 Ti for +4 HP. A sentinel shot is 10 ammunition -- 10 Ti -- for 18
# damage, which is 1.8 HP per titanium. So HEALING IS 2.2x MORE TITANIUM
# EFFICIENT THAN SHOOTING, and in any game that does not end in the opening
# burst, the side that heals beats the side that shoots on identical income.
#
# That is exactly how not adgato beats us in the games our rush fails to close.
# Traced on an 18x18 game we lost at t317: our four sentinels put their core to
# 80 HP by t40 and both banks ran dry. They then parked a builder on their core
# and healed it 80 -> 498 while banking the surplus (their titanium climbed 6 ->
# 61); our core sat at 284 and never healed at all, because we were converting
# every scrap of passive income straight into ammunition and dribbling it at
# 4.5 HP a turn into a core they were repairing at 5.2. We spent our entire
# income for a net negative. Then they burst us down.
#
# So: if the rush has not closed and we are being shot, stop shooting and start
# repairing. Spawned LATE and conditionally, which is what makes it nearly free
# -- the global cost scale is what killed every previous second-builder arm
# (economy 19.8-39.5%, medic 27.9%), but by HEALER_ROUND the battery is already
# standing and there is nothing left whose price a +20% scale can raise.
HEALER_ENABLED = True
HEALER_ROUND = 70                # not before this: fast games must stay free
HEAL_RESERVE = 45                # titanium the core stops converting, to heal with

# Sentinels prefer enemy builder bots to the enemy core -- see sentinel_turn.
SENTINEL_HUNTS_BUILDERS = True
TARGET_RANK_BUILDER = -1         # above CORE, which is rank 0

# The barrier ring -- see _try_barrier_ring. Eight 3-titanium walls on the tiles
# a builder would have to stand on to repair the enemy core, attempted only when
# nothing at all is defending it and only when we can walk the ring without
# treading on the tiles we are filling in.
BARRIER_RING = True
BARRIER_BEFORE_SENTINELS = False

SLOT_RAIDER_SEEN = 6             # the rusher saw a lone enemy builder crossing

# Defensive healers. The rush opponent sends ONE builder; if it reaches our core
# it is there to plant a battery, and the answer is not to fight it -- a builder
# attack is 2 titanium for 2 damage and an enemy sentinel ignores obstacles and
# out-ranges everything we could do -- but to out-repair it. Two healers on the
# core is +8 HP per turn for 2 titanium against a four-sentinel battery's 36 per
# turn for 20, and their battery runs out of ammunition long before our income
# runs out of repairs.
HEALER_MAX = 2

# Far face, conditional -- see _pick_stand. The bonus above applies once a
# defender has been SEEN near the target core; with nobody home the fast route
# wins instead.
FAR_FACE_BONUS_CLEAR = 0.0
FAR_FACE_WATCH_GAP = 12          # close enough that a builder here is a defender

SLOT_BASE_EMPTY = 7              # rusher reached their core and nothing was home

# How far the core must be down before a healer is worth its price.
#
# By the time the battery is standing the cost scale is 2.0, so a builder is 60
# titanium -- six sentinel shots, 108 damage we will not deal. Healing is worth
# more per titanium (4 HP against 1.8) but only if there is damage to undo, so
# paying the moment the core takes a scratch buys an expensive builder in games
# we were winning anyway.
HEALER_HP_FRACTION = 0.65

# Step around a blocker rather than standing in front of it -- see _step_toward.
# Ablatable because it cuts both ways: it is what stops the thousand-turn
# deadlock, but yielding a corridor also hands the race to whoever stays put,
# and herbert19 body-blocks on purpose.
SIDESTEP_BLOCKED = True

# Turns to stand firm against a blocker before walking around it -- see
# _step_toward. 0 would yield instantly (-10 points against a bot that
# body-blocks); never yielding is a thousand-turn deadlock.
SIDESTEP_AFTER = 8

# After the first sentinel, build from where we stand rather than relocating.
BUILD_BEFORE_MOVING = True

# Turns of a builder neither moving nor building before we assume something has
# gone wrong and just walk at the enemy core -- see panic().
STUCK_LIMIT = 12

# Wait for defenders to leave before planting the FIRST sentinel -- see
# rusher_turn. not adgato opens at a nearest-defender distance of 5, 6, 6, 5, 3;
# we were opening at 2, 1, 2, 3 and losing the turrets one at a time.
# OPEN_ANYWAY_AFTER already bounds the wait, so this cannot stall forever.
STALL_UNTIL_CLEAR = True
OPEN_CLEAR_DIST = 4

# Hard cap on turns spent waiting for a clearing before the first sentinel.
# not adgato opens at t30-44 against ph; uncapped, we were opening at t119-128.
STALL_MAX_TURNS = 15

# How many enemy builders must be inside OPEN_CLEAR_DIST before waiting is worth
# the tempo -- see rusher_turn. ph fields 2.34 within four on average, herbert19
# 0.94.
STALL_MIN_DEFENDERS = 2

# Standing beside an enemy launcher is a loop, not a risk -- see _launcher_grip.
LAUNCHER_GRIP_PENALTY = 40.0

# Throws we will tolerate before routing around every enemy launcher.
THROWS_BEFORE_AVOID = 1
