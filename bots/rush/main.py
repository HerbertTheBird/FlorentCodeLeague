...
class Player:
    ...
    def run(self, ct: Controller) -> None:
        try:
            # Entity kind FIRST, and independently of the rest of init.
            #
            # This used to be set after State was built, inside the same `if
            # self.st is None` block. Anything throwing in between left self.st
            # assigned and self.kind None -- and because the guard is on st, init
            # never ran again, every dispatch branch below compared against None,
            # and the unit did NOTHING FOR THE REST OF THE GAME. That is exactly
            # the observed failure against ph: the rusher walked two steps and
            # then stood on (5,13) from t3 to t151 while we lost 0-5.
            ...
            if self.kind == EntityType.CORE:
                ...
                elif config.ECON_ENABLED:
                    roles.eco_turn(ct, self.st)
                else:
                    # Nothing but the rusher and healers exists, so a builder
                    # that is not the rusher is a healer. No id slot needed.
                    roles.healer_turn(ct, self.st)
            ...
            try:
                ...
                pass
            # Having thrown, do not ALSO stand still. A turn spent walking at the
            # enemy core is never worse than a turn spent frozen.
            try:
                roles.panic(ct, self.st)
            except Exception:
                pass
