...
def _entry_matches(spec, role: int, prog: list, me) -> bool:
    ...
    if role < len(script) and opener.pos(script[role][1]) == me:
        return True
    return (bool(prog) and prog[0][0] == openers.WAIT
            and not isinstance(prog[0][1], str)   # a RELAY names no landing tile
            and opener.pos(prog[0][1]) == me)


def _throw_possible(target=None) -> bool:
    ...
    `target` of None asks only the first half -- "can anything pick me up at
    all", which is what a RELAY wait is asking. A relay's remaining hops are not
    something the builder can count, but the launcher standing next to it is
    proof there is at least one left.
    ...
    for p in map_info.iter_mask(mask):
        if p.distance_squared(me) > 2:
            continue
        if target is None or p.distance_squared(target) <= 26:
            return True
    return False
...
def _claim() -> None:
    ...
    for role, prog in enumerate(spec["builders"]):
        ...
        if (prog and prog[0][0] == openers.WAIT
                and not isinstance(prog[0][1], str)
                and opener.pos(prog[0][1]) != me
...
def _ferry_eligible() -> bool:
    """Is this builder one the launchers may send forward?

    A scripted role never is, whatever its id: it has a post to hold, and the
    whole point of the guards is that they stay on the barriers they built.
    """
    if _ferried or _prog is not None:
        return False
    if _ferrying:
        return True
    return opener.ferry_wants(comms._my_slot, rc.get_id())


def _gate_open() -> bool:
    """Has the map's `ferry_after` condition been met?

    Latched, both forms. "After we finish eco" and "after we pass 1000 titanium"
    are phase changes, and both readings go false again on their own -- a
    harvester dies, or the bank gets spent -- so read live they would let only
    the builders that happened to evaluate on a true turn ever go.
    """
    global _eco_done
    if _eco_done:
        return True
    after = opener.spec.get("ferry_after")
    if after == "ore_worked":
        return _ore_worked()
    if isinstance(after, dict) and "titanium" in after:
        if rc.get_global_resources() <= after["titanium"]:
            return False
        _eco_done = True
        return True
    return False
...
def _scouting():
    ...
    Only builders whose decision depends on the answer bother. Anyone who is
    never going forward has no reason to walk across the map to confirm
    something it will not act on.
    """
    if not _ferry_eligible() or _eco_done:
        return None
    unseen = _unseen_ore()
    if not unseen:
        return None
...
def _across() -> bool:
    ...
    mine = map_info._my_core
    if theirs is None or mine is None:
        return False
    me = map_info._my_pos
    return me.distance_squared(theirs) < me.distance_squared(mine)
...
def _ferry_route():
    ...
    if not stops:
        return None
    me = map_info._my_pos
    mine = [(s_, d_) for s_, d_, who in stops
            if opener.split_matches(comms._my_slot, rc.get_id(), who)]
    if not mine:
        return None
    for src, dst in mine:
        if src == me:
...
def _update_ferry_avoid() -> None:
    ...
    if opener.spec is None:
        ...
        return
    w = map_info._width
    # Nobody stands on a launcher site, ever. A builder parked on one makes the
    # tile unbuildable, and the builder waiting beside it to build there has no
    # way to ask it to move.
    for site in opener.ferry_sites():
        mask |= 1 << (site.x + site.y * w)
    if not _ferry_eligible():
        for src, _, _w in opener.ferry_stops():
            mask |= 1 << (src.x + src.y * w)
...
def score():
    ...
    if not _claimed:
        _claim()
    # Before the guard branch returns, not after: the scripted guards are exactly
    # the builders that must never wander onto a pickup square, and a launcher a
    # guard is standing next to would otherwise throw it off its own barrier.
    _update_ferry_avoid()
    ...
    if not _ferried:
        # Two ways to know. A named destination can be compared outright; a
        # dynamic one (`enemy_side`) has no tile to compare against, so the test
        # is the property the throw was for -- are we on their half now. The
        # second also correctly rejects royale's mid-chain landings, which are
        # still nearer our own core than theirs.
        for _, dst, _w in opener.ferry_stops():
            if not isinstance(dst, str) and map_info._my_pos == dst \
                    and opener.ferry_terminal(dst):
                _ferried = True
                break
        if _ferrying and not _ferried and _across():
            _ferried = True
        if _ferried:
...
def _enemy_building_at(tile) -> bool:
    """Is an enemy building standing on `tile`, as far as we can tell?

    A tile out of this builder's r^2=20 answers None and so reads as clear,
    which is why the tiles a SKIP_UNLESS_ENEMY names have to be ones the builder
    will be able to see when it reads them.
    """
...
def run():
    ...
    while _step < len(_prog):
        ...
        if kind == openers.WAIT:
            ...
            if op[1] == openers.RELAY:
                # A relay ends where the launchers do. Asking "am I on the last
                # landing tile" cannot work -- two of these throws land in the
                # same round, so there are tiles this builder passes through
                # without ever running on one -- but "can anything still pick me
                # up" is answered by looking around, and it is the same
                # question. Every mid-relay tile is a pickup square by
                # construction, because a builder only ever reaches one by
                # landing where the builder ahead of it was thrown from.
                if _throw_possible():
                    break
                _step += 1
                continue
        ...
        if kind == openers.SKIP_UNLESS_ENEMY:
            tiles = op[1] if isinstance(op[1][0], (tuple, list)) else (op[1],)
            if any(_enemy_building_at(opener.pos(t)) for t in tiles):
                _step += 1
            else:
                _step += 1 + op[2]
                log(f"OPENER role {_role} skips {op[2]} op(s): {tiles} clear")
            continue
