...
_core_pos = None             # non-core: our core's tile, from slot 0
...
def slot_at(tile: Position) -> int | None:
    """Which builder slot is broadcasting `tile` as its position, if any.

    The slot index is the only per-TEAM ordinal a unit can learn about another
    unit. Entity ids come from one counter shared by both teams, so any function
    of an id -- parity, or a hash -- inherits whatever interleaving the two teams
    happened to produce, and on a fifteen-builder sample that skew is large: odd
    ids selected 25% of our builders on one side of royale and 60% on the other.
    Slots are claimed lowest-free-first from our own fourteen, so slot parity is
    exactly seven and seven whatever the ids look like.

    Costs one `read_store` per slot and is meant for launchers, which have no
    `_my_slot` of their own and never call `read()`. Positions are a round stale,
    which is harmless for a builder that has stopped on a pickup tile -- it is
    still there next turn.
    """
    for s in range(_FIRST_BUILDER_SLOT, _DEFENSE_SLOT):
        val = rc.read_store(s)
        if not val or not ((val >> _VALID_SHIFT) & 1):
            continue
        n = val & _POS_MASK
        if n % _width == tile.x and n // _width == tile.y:
            return s
    return None


def core_position() -> Position | None:
    """Our core's top-left tile, as broadcast in slot 0.

    `map_info._my_core` is only set once a unit has actually SEEN the core, and
    two kinds of unit may never do so: a builder the core spawns and a launcher
    throws on that same turn is already across the map by its first `run()`, and
    a turret built at the far end of the map never had it in vision at all. The
    core puts its position in slot 0 every round regardless.

    Falls back to reading slot 0 directly for units that never call `read()` --
    the turrets. That is one `read_store` and none of `_absorb`'s side effects,
    which is the point: a sentinel should not have to start absorbing relayed
    terrain and ageing heartbeats just to learn where home is.
    """
    if _core_pos is not None:
        return _core_pos
    val = rc.read_store(_CORE_SLOT)
    if not val or not ((val >> _VALID_SHIFT) & 1):
        return None
    n = val & _POS_MASK
    return Position(n % _width, n // _width)
...
def _absorb():
    global _route_total, _core_route_total, _core_repair_target, _core_pos
    dirty = False
