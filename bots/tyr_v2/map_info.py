...
# to the front from wandering onto a ferry launcher's pickup square.
_bm_avoid_extra: int = 0
...
def update_at(pos: Position) -> None:
    ...
    # Call for every conveyor-like (symmetric with the removal paths above): a
...
_known_map_applied = False

def apply_known_map(wall: int, ore: int, sym: str, enemy_core: Position | None) -> None:
    """A mapdata entry has been uniquely identified: fill map_info with the full,
    exact board so no unit has to rediscover any of it. Fills the env for EVERY
    tile (walls, ore, else empty), marks the whole board seen, resolves the
    symmetry, and pins the enemy core (position + 2x2 area). Idempotent -- runs its
    work only once."""
    global _known_map_applied
    global _bm_seen, _hor_sym, _ver_sym, _rot_sym, _solved_sym
    global _their_core, _predicted_enemy_core, _bm_their_core_area
    if _known_map_applied:
        return
    _known_map_applied = True
    board = _board_mask
    wall &= board
    ore &= board
    # Full env for the whole board.
    _bm_env[_IDX_ENV_WALL] = wall
    _bm_env[_IDX_ENV_ORE_TI] = ore
    _bm_env[_IDX_ENV_EMPTY] = board & ~wall & ~ore
    # The entire board is now known.
    _bm_seen = board
    # Symmetry is resolved.
    _hor_sym = (sym == "h")
    _ver_sym = (sym == "v")
    _rot_sym = (sym == "r")
    _solved_sym = bool(sym)
    # Enemy core.
    if enemy_core is not None:
        _their_core = enemy_core
        _predicted_enemy_core = enemy_core
        ecm = 0
        for x in range(enemy_core.x, enemy_core.x + 2):
            for y in range(enemy_core.y, enemy_core.y + 2):
                if 0 <= x < _width and 0 <= y < _height:
                    ecm |= 1 << (x + y * _width)
        _bm_their_core_area = ecm
