...
def _narrow() -> None:
    ...
    if _cands is None:
        ...
        for entry in mapdata.MAPS.get((map_info._width, map_info._height), ()):
            ...
            if mine != core_a and mine != core_b:
                continue
            _cands.append((nm, sym, int(wall_hex, 16), int(ore_hex, 16), core_a, core_b))
        if not _cands:
    ...
    # The moment the board matches exactly one known map, pour the whole thing into
    # map_info -- full env, all tiles seen, symmetry, enemy core -- regardless of
    # whether this map also has a hardcoded opener to run.
    if len(_cands) == 1:
        nm, sym, wall, ore, core_a, core_b = _cands[0]
        enemy = core_b if mine == core_a else core_a
        map_info.apply_known_map(wall, ore, sym, Position(*enemy))
