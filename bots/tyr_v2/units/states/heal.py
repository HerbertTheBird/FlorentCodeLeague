...
_cached_service_target: Position | None = None
_cached_service_tiles: set[Position] | None = None
_cached_service_step: Position | None = None
_cached_service_urgent = False
...
def _service_plan(urgent: bool) -> tuple[Position, set[Position], Position] | None:
    """Choose a damaged building and a reachable cardinal service tile.

    Healing targets the building, but builders must stand on a *different*,
    cardinally adjacent passable tile. Searching the building mask directly
    made blocked structures look reachable because pathing deliberately allows
    its destination tile into the BFS graph. Build the legal standing-tile mask
    first and search it once; unreachable buildings contribute no reachable
    service tile, so the search naturally falls through to another target.
    """
    targets = _service_targets(urgent)
    if not targets:
        return None
    w = map_info._width
    ...
    for target in map_info.iter_mask(targets):
        for d in map_info._CARDINAL:
            tile = map_info.pos_add(target, d)
            if not map_info.in_bounds(tile) or not map_info.is_passable(tile):
                continue
            n = tile.x + tile.y * w
            if n != my_n and occupied & (1 << n):
                continue
            service_mask |= 1 << n
            service_tiles.add(tile)
    if not service_mask:
        return None

    # Use the same movement search as move_to(), including live bot blockers and
    # the urgent/non-urgent turret policy. A cheaper geometric BFS can report a
    # path through another builder and recreate the original stuck behavior.
    route = nav.bfs_move(
        my_n, service_mask, map_info.get_avoid(False),
        avoid_turret=not urgent,
    )
    if route is None:
        log("heal: no reachable service tile; giving up")
        return None

    # Retain a representative target for diagnostics; the BFS above chose the
    # actual next step toward the nearest reachable service destination.
    best_target = None
    best_score = -1
    for target in map_info.iter_mask(targets):
        n = target.x + target.y * w
        et_idx = map_info._building_et_idx[n]
        if et_idx < 0:
            continue
        damage = map_info._MAX_HP_BY_IDX[et_idx] - map_info._building_hp[n]
        value = damage * _HEAL_PRIORITY[et_idx]
        if value > best_score:
            best_score = value
            best_target = target
...
def score(can_move=True):
    ...
    _cached_service_target = None
    _cached_service_tiles = None
    _cached_service_step = None
    _cached_service_urgent = False
    plan = _service_plan(True)
    if plan is not None:
        _cached_service_target, _cached_service_tiles, _cached_service_step = plan
        _cached_service_urgent = True
        return MAX_SCORE
    plan = _service_plan(False)
    if plan is not None:
        _cached_service_target, _cached_service_tiles, _cached_service_step = plan
...
def run(can_move=True):
    ...
    log("heal: service", "urgent" if _cached_service_urgent else "normal",
        _cached_service_target, "from", len(_cached_service_tiles), "legal service tiles")
    if _cached_service_step != map_info._my_pos:
        nav.move(map_info.direction_to(map_info._my_pos, _cached_service_step))
    _do_best_heal()
