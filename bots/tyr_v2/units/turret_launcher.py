...
def run():
    ...
    if _try_throw_enemy_away():
        return
    # A hardcoded opener's scripted throws sit here, below enemy removal: an
    # enemy builder standing next to us is still the more urgent thing, and the
    # script is written to be retried, so losing a turn to that costs a turn.
    if opener.launcher_throw():
        return
