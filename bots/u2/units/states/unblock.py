...
def _severed_assets() -> int:
    """Our harvesters that are NOT connected to our core by our own conveyors.

    Floods our conveyor graph out from our core; any harvester the flood does not
    touch is producing titanium that never arrives. That is the asset worth
    breaking a blocker for.
    """
    core = map_info._bm_my_core_area
    if not core:
        return 0
    mine = map_info._bm_team[map_info._my_team_idx]
    harvs = map_info._bm_et[map_info._IDX_HARVESTER] & mine
    if not harvs:
        return 0
    convs = map_info._bm_et[map_info._IDX_CONVEYOR] & mine
    seen = core
    for _ in range(60):
        nxt = map_info.manhattan(seen) & (convs | harvs) & ~seen
        if not nxt:
            break
        seen |= nxt
    return harvs & ~seen
...
def _frontier_obstacles():
    ...
    # #51 PER-LINE SEVERANCE. The old gate was `if strict & targets: return 0` --
    # unblock stood down whenever ANYTHING was reachable anywhere. Ladder match
    # 484a6bbe game 1: an enemy barrier on (7,3) sat at 30/30 HP from t16 to t432,
    # never once attacked, stranding our (7,2) harvester all game and losing on
    # economy -- because other route work was reachable, so unblock declined.
    # A harvester delivers 2.5 Ti/round forever and a barrier is 30 HP = 15
    # builder-turns, so freeing ONE severed line repays in ~12 rounds.
    if strict & targets:
        # Something is reachable, but do we own an asset that is CUT OFF and that
        # breaking would reconnect? If not, this really is not our job.
        cut_off = _severed_assets()
        if not cut_off:
            return 0
        if not (relaxed & cut_off) or (strict & cut_off):
            return 0
