...
# Heimdall_v2 opening: 3 attackers, 2 economy, no defenders.
_atk_ids = [0, 0, 0]
_econ_ids = [0, 0]
...
def _record_opening_spawn(spawn_index: int, builder_id: int) -> None:
    """Record the opening role (attack, attack, attack, economy, economy) for
    this builder. The actual mailbox writes happen once per round in run()."""
    global _num_spawned
    if spawn_index in (0, 1, 2):
...
def _spawn_toward_plan(core_pos: Position) -> bool:
    ...
    # Heimdall_v2: 3 attackers get the three lead rays; the 2 economy builders
    # get the farthest rays. No defenders, so no launcher-adjacent spawning.
    if spawn_index in (0, 1, 2):
        planned_dir = _spawn_plan[min(spawn_index, len(_spawn_plan) - 1)]
...
def run():
    ...
    # Rebroadcast the 3-attack / 2-economy opening role ids (buffered writes need
    # repeating until every builder has recognized its assignment).
    if any(_atk_ids) or any(_econ_ids):
        comms.rebroadcast_opening(_atk_ids, _econ_ids)
