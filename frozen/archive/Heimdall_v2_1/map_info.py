...
def atk_symmetry_target(atk_index):
    """Enemy-core target for attack builder ``atk_index``.
      - Before the symmetry is known, both attackers assume the DIAGONAL
        (rotational) core.
      - If rotational is eliminated, both use the horizontal guess while both
        axes remain possible, keeping the v2.1 siege pair together.
    ...
    if _hor_sym and _ver_sym:
        # v2.1's two siege builders stay paired instead of splitting across
        # symmetry guesses.  Both the builders and their launchers call this
        # helper, so travel and throws remain coordinated as well.
