...
def _turret_covered_mask():
    return map_info._bm_my_gunner_claims
...
def _try_launcher_fallback():
    ...
    convs = (map_info._bm_et[map_info._IDX_CONVEYOR]) & map_info._bm_team[my_team_idx]
    ...
    enemy_zone_4 = map_info.expand_manhattan(enemies, 4)
    ...
    while em:
        ...
        t0_mask = map_info.expand_manhattan(t1_mask) & ~t1_mask & passable  # stand tiles: builds are cardinal-only
        if not t0_mask:
        ...
        t1_neighbors = map_info.expand_manhattan(1 << t0_n) & ~(1 << t0_n) & t1_mask  # cardinal build reach
        if not t1_neighbors:
...
def _find_chase_target_main(damaged: bool = True):
    ...
    nearby = filtered & map_info.expand_manhattan(my_bit, 8)  # walk reach
    if not nearby:
        log("too far")
        return None
...
def _mutual_sentinel_threat():
    ...
    my_sents &= ~map_info.expand_manhattan(map_info._bm_friendly_bots)  # heal reach is cardinal
...
def _conv_dist(pbit: int, source: int, cap: int = 12) -> int:
    ...
    for d in range(1, cap + 1):
        cur = map_info.expand_manhattan(cur)  # movement hop distance
        if cur & pbit:
            return d
    return cap + 1
...
def _try_chase(target):
    ...
    if _cached_launcher_t1 is not None:
        ...
        t1_n = t1.x + t1.y * w
        t0_zone = map_info.expand_manhattan(1 << t1_n) & ~(1 << t1_n) & ~avoid  # stand tiles: cardinal build reach
        t0_set = set()
        m = t0_zone
        while m:
            lsb = m & -m
        ...
        if t0_set:
            nav.move_to(t0_set)
        my_pos = map_info._my_pos
        if my_pos.distance_squared(t1) <= 1 and my_pos != t1:  # Titan: cardinal build reach
            # Loki: gunners-only by CHOICE (buildability untouched). Khaos picked a
            # launcher here (throw the harasser away) unless T1 was a guard
            # conveyor; guard conveyors are gone in loki, so always place a gunner
            # facing the chased enemy instead.
            gunner_dir = t1.direction_to(target)
            if rc.can_build_gunner(t1, gunner_dir) and rc.get_global_resources() >= rc.get_gunner_cost() + max(map_info.builder_ti_reserve(), 8):  # keep >= 8 ti after a gunner
                ...
                log("heal: gunner build blocked at", t1, "(can_build=", rc.can_build_gunner(t1, gunner_dir), "ti=", rc.get_global_resources(), "need=", rc.get_gunner_cost() + map_info.builder_ti_reserve(), ")")
        _do_best_heal()
        return True
