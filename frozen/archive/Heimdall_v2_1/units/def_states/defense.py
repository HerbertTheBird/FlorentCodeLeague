...
def _block_approaching_enemy() -> bool:
    """When we can't build a launcher, block the nearest enemy bot closing on our
    core with a barrier (or approach the tile to bodyblock) so a broke/ring-done
    defender still contains the intruder instead of freezing.

    Prefer the dominant-axis coreward tile, but if that's occupied (a wall or one
    of our own ring launchers), fall back to any empty cardinal neighbour of the
    enemy that is strictly closer to the core — the tile it would actually step
    onto to flank around. If every coreward step is already blocked, the enemy is
    contained and there is nothing useful to do. Returns True if it claimed the
    turn."""
    ...
    if enemy_pos is None:
        return False
    enemy_d = _distance_from_core(enemy_pos)
    if enemy_d > 36:
        return False
    candidates = []
    primary = coreward_block_tile(enemy_pos)
    if _empty_unoccupied(primary):
        candidates.append(primary)
    for direction in _CARDINALS:
        p = map_info.pos_add(enemy_pos, direction)
        if (
            map_info.in_bounds(p)
            and _distance_from_core(p) < enemy_d
            and _empty_unoccupied(p)
        ):
            candidates.append(p)
    if not candidates:
        return False
    # Block the coreward step we can reach fastest.
    tile = min(candidates, key=lambda p: p.distance_squared(map_info._my_pos))
    return _build_or_approach_barrier(tile)
...
def _pure_mirror_enemy(enemy: Position | None, reported: Position | None) -> bool:
    """Copy one observed cardinal delta exactly.

    Return False once the target has been lost, jumped identities/positions, or
    made a move we cannot mirror.  That is the signal for a permanent defender
    to give up the claim and return to its home launcher.
    """
...
def _release_lane_to_home(lane: int) -> None:
    """Make this defender launchable again and start walking home immediately."""
    global _mirror_enemy_position
    _mirror_enemy_position = None
    if _first_launcher_id:
        comms.publish_defender_home(lane, rc.get_id(), _first_launcher_id)
    _hold_first_launcher()


def _mirror_lane_intruder(lane: int) -> bool:
    """Mirror this lane's intruder while cardinally containing it.

    A stale sighting, separation beyond the one-step (two-tile) transient gap,
    or blocked mirror movement ends the assignment. The defender then returns
    to its first launcher and becomes available for the next claim.
    """
    ...
    if enemy is None:
        enemy = reported
    if enemy is None or max(
        abs(enemy.x - map_info._my_pos.x),
        abs(enemy.y - map_info._my_pos.y),
    ) > 2:
        _release_lane_to_home(lane)
        return True
    if not _pure_mirror_enemy(enemy, reported):
        _release_lane_to_home(lane)
    return True
...
def run(lane: int) -> bool:
    ...
    if _gunner_pvp_mode:
        ...
        _run_gunner_pvp()
        return False
    # Decide whether we'll actually place a launcher this turn: we need a ring
    # site for our lane (plan non-empty with a next site) and enough titanium.
    # If not — ring already complete, no lane site yet, or broke — and an enemy
    # is closing on the core, block it with a barrier instead of holding or
    # freezing at a spot we can't build on.
    target = next_launcher_site(lane) if plan else None
    if (
        (
            target is None
            or rc.get_global_resources()
            < rc.get_launcher_cost() + map_info.builder_ti_reserve()
        )
        and _block_approaching_enemy()
    ):
        return False
    if not plan:
        _target, uncovered = _uncovered()
