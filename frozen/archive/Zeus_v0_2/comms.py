...
# Slot 15: the spawn turn (+1, so 0 = unclaimed) of the ONE "speedup" launcher —
# the first launcher built. It flings the opening roster across the map. Every
# later launcher is a killbox launcher and never launches friendly builders.
SPEEDUP_LAUNCHER_SLOT = 15


def claim_speedup_launcher(spawn_turn: int) -> bool:
    """First launcher to call this claims the speedup role and gets True. Any
    later launcher sees it already claimed and gets False (killbox launcher)."""
    if rc.read_store(SPEEDUP_LAUNCHER_SLOT):
        return False
    rc.write_store(SPEEDUP_LAUNCHER_SLOT, (spawn_turn + 1) & 0xFFFFFFFF)
    return True
