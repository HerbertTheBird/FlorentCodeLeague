...
_bm_killbox_clear: int = 0       # killbox + adjacent; kept clear of routes/harvesters (set by builder)
...
def _compute_my_gunner_claims() -> int:
    ...
    claimed = 0
    if gunners:
        # A gunner ray is stopped by walls AND buildings (barriers etc.), not just
        # walls — otherwise a gunner whose shot is sealed behind its own barrier
        # (e.g. a killbox gunner) claims tiles it can never reach, walling a
        # builder into a dead pocket. (A ray never includes its own gunner tile,
        # so blocking on all buildings is safe here.)
        from _config import GUNNER_CLAIMS_STOP_AT_BUILDINGS
        not_blocked = _board_mask & ~_bm_env[_IDX_ENV_WALL]
        if GUNNER_CLAIMS_STOP_AT_BUILDINGS:
            not_blocked &= ~_bm_any_building
        for di in range(8):
            dm = gunners&_bm_dir[di]
            if not dm:
                continue
            ...
            for i in range(1, length+1):
                offset = dx + dy * w
                if offset > 0:
                    dm = ((dm & shift_mask) << offset) & not_blocked
                else:
                    dm = ((dm & shift_mask) >> (-offset)) & not_blocked
                claimed |= dm
...
def _insert_core_building(core: Position, team_idx: int) -> None:
    """Write a single core ORIGIN tile into the raw building/team arrays so
    build_core_areas() can expand it to the 2x2 footprint. Mirrors the enemy-
    core insertion in update(); used by load_full_map() for a core we know from
    the identified map but haven't seen locally."""
    global _bm_any_building
    n = core.x + core.y * _width
    bit = 1 << n
    _building_id[n] = -1
    _building_et_idx[n] = _IDX_CORE
    _building_hp[n] = GameConstants.CORE_MAX_HP
    _bm_et[_IDX_CORE] |= bit
    _bm_team[team_idx] |= bit
    _bm_any_building |= bit


def _solve_symmetry_from_board() -> None:
    """Set the symmetry flags to the exact symmetries the (now fully known)
    wall+ore layout satisfies, and mark the symmetry solved. Enemy-core targeting
    no longer depends on these once _their_core is known, but keeping them exact
    keeps atk_symmetry_target / local_solved_symmetry_code consistent."""
    global _hor_sym, _ver_sym, _rot_sym, _solved_sym
    w = _width
    h = _height
    feat = _bm_env[_IDX_ENV_WALL] | _bm_env[_IDX_ENV_ORE_TI]
    hor = ver = rot = True
    m = feat
    while m:
        lsb = m & -m
        ...
        x = n % w
        y = n // w
        if hor and not (feat >> ((w - 1 - x) + y * w)) & 1:
            hor = False
        if ver and not (feat >> (x + (h - 1 - y) * w)) & 1:
            ver = False
        if rot and not (feat >> ((w - 1 - x) + (h - 1 - y) * w)) & 1:
            rot = False
        if not (hor or ver or rot):
            break
    _hor_sym, _ver_sym, _rot_sym = hor, ver, rot
    _solved_sym = True


def load_full_map(width: int, height: int, walls: int, ore: int,
                  my_core: Position | None, their_core: Position | None) -> None:
    """Load an identified map wholesale: every wall/ore tile, both cores, the
    solved symmetry, and mark the whole board seen. After this a unit has
    complete map knowledge with no exploration or comms tile-pooling.

    Env/seen and any core we didn't already know are authoritative overwrites
    (the map file is ground truth); live building state keeps flowing through
    update_at(), whose core-area guard protects the loaded core tiles."""
    global _bm_seen, _struct_version, _core_id
    global _my_core, _their_core, _predicted_enemy_core
    if width != _width or height != _height:
        return
    board = _board_mask
    _bm_env[_IDX_ENV_WALL] = walls & board
    _bm_env[_IDX_ENV_ORE_TI] = ore & board
    _bm_env[_IDX_ENV_EMPTY] = board & ~(_bm_env[_IDX_ENV_WALL] | _bm_env[_IDX_ENV_ORE_TI])
    _bm_seen = board

    if _my_core is None and my_core is not None:
        _my_core = my_core
        if _core_id is None:
            _core_id = -1
        _insert_core_building(_my_core, _my_team_idx)
    if _their_core is None and their_core is not None:
        _their_core = their_core
        _insert_core_building(_their_core, 1 - _my_team_idx)

    _solve_symmetry_from_board()
    build_core_areas()
    if _their_core is not None:
...
def loaded_route_reaches_core() -> bool:
    """True when one of our titanium-carrying conveyors has a downstream chain
    reaching our core — i.e. an economy supply line is actually delivering
    resource to the core."""
    reaches_core, _ = _compute_route_reaches_core()
    return bool(reaches_core & _bm_conv_ti)


def my_route_complete() -> bool:
    """True when we have conveyors and EVERY one of them has a downstream chain
    reaching our core — i.e. no route is left dangling. Structural (independent
    of titanium load), so it is not fooled by the transient claim lull while a
    frontier conveyor waits to load."""
    my_conv = _bm_conveyors & _bm_team[_my_team_idx]
    if not my_conv:
        return False
    reaches_core, _ = _compute_route_reaches_core()
    return (my_conv & ~reaches_core) == 0
...
def is_passable(pos: Position):
    ...
    from _config import BARRIER_WALKTHROUGH_ILLEGAL
    walkable = _bm_et[_IDX_CONVEYOR] | _bm_et[_IDX_SPLITTER]
    if not BARRIER_WALKTHROUGH_ILLEGAL:
        walkable |= _bm_et[_IDX_BARRIER] & _bm_team[_my_team_idx]  # own barriers break-through-able
    return bool(walkable & bit)
...
def get_avoid(
    ...
    if avoid_conveyors:
        ...
        # Never route conveyors through the killbox or the tiles adjacent to it.
        mask |= _bm_killbox_clear
    if avoid_ore:
