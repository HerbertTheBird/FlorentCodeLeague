...
class Pathing:
    ...
    def move(self, dir: Direction):
        ...
        # Never walk through (destroy) a FRIENDLY barrier — that's illegal. Enemy
        # barriers may still be broken through to advance. (When the toggle is
        # off, revert to the baseline: any barrier is break-through-able.)
        from _config import BARRIER_WALKTHROUGH_ILLEGAL
        if (id and rc.get_entity_type(id) == EntityType.BARRIER
                and (not BARRIER_WALKTHROUGH_ILLEGAL or rc.get_team(id) != rc.get_team())
                and rc.can_destroy(new_pos) and rc.get_action_cooldown() == 0):
            rc.destroy(new_pos)
    ...
    def bfs_move(self, start_n: int, target_mask: int, avoid: int, avoid_turret: bool = True):
        ...
        avoid &= ~start_mask
        # Friendly barriers block movement — never plan a path through (or
        # destroy) one. (Walking through your own barrier is illegal.)
        from _config import BARRIER_WALKTHROUGH_ILLEGAL
        if BARRIER_WALKTHROUGH_ILLEGAL:
            avoid |= bm_et[idx_barrier] & bm_team[map_info._my_team_idx] & ~start_mask
    ...
    def move_to(
        ...
        if allow_enemy_gunner:
            ...
            # gunner lanes, while permitting that deliberate risk. Only lift the
            # threat-avoidance for tiles that are otherwise WALKABLE — never for
            # structurally impassable tiles (walls / cores / buildings), which an
            # enemy ray may cross: un-avoiding those makes the pathfinder route
            # straight into them, the move fails every turn, and the unit stalls
            # (this stranded killbox builders whose nest hugs a fire-swept core).
            avoid &= ~(map_info._bm_enemy_hard_threat & ~map_info._bm_blocked)
        # if avoid_empty:
