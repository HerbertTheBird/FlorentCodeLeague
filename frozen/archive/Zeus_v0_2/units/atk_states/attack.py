...
def _threshold_attack_candidates(gunner_masks, gun_planes_by_dir):
    """Tiles surviving Zeus's max-score-relative placement threshold."""
    gunner_any = 0
...
def _facing_at_friendly_builder(pos, direction) -> bool:
    """True if a gunner at `pos` facing `direction` would have a friendly builder
    bot on its firing line (before any wall/building). We never place a gunner
    like that — it would trap the friendly bot in the ray (it avoids our gunner
    rays) with nowhere to go."""
    from _config import ATTACK_AVOID_FRIENDLY_FACING
    if not ATTACK_AVOID_FRIENDLY_FACING:
        return False
    if direction == Direction.CENTRE:
        return False
    di = map_info._DIR_INT[direction]
    if di >= 8:
        return False
    w, h = map_info._width, map_info._height
    walls = map_info._bm_env[map_info._IDX_ENV_WALL]
    buildings = map_info._bm_any_building
    ...
    for dx, dy in map_info._GUNNER_RAYS[di]:
        ...
        if friendly & bit:
            return True
        if buildings & bit:
            break  # a building stops the ray before anything further
    return False
...
def get_best_direction(pos):
    ...
    for d in range(8):
        if not (gunner_masks[d] & bit):
            continue
        if _facing_at_friendly_builder(pos, directions[d]):
            continue  # never aim a gunner at a friendly builder
...
def run():
    ...
    # Among this builder's claimed candidates, pick the HIGHEST-SCORING tile. The
    # gunner score already bakes in the enemy-fire threat penalty (tiles under
    # enemy fire score lower) and the distance discount (tiles closer to enemy
    # targets score higher), so the raw score is exactly what to maximise — no
    # separate race/lead heuristic. Seed with nav.closest's score and override
    # only on a strictly higher score, so ties keep the nearest tile.
    _, _, best_sc = get_best_direction(best)
    remaining = preferred & ~(1 << (best.x + best.y * width))
    while remaining:
        lsb = remaining & -remaining
        n = lsb.bit_length() - 1
        remaining ^= lsb
        pos = Position(n % width, n // width)
        _, _, sc = get_best_direction(pos)
        if sc > best_sc:
            best_sc = sc
            best = pos
    ...
    if not _defensive():
        ...
        if core_facing is not None and not _facing_at_friendly_builder(best, core_facing):
            direction = core_facing
    log(f"Attack: best={best}, dir={direction}")
    nav.move_adjacent(best)
    # Never aim a gunner at a friendly builder (it would trap it in the ray).
    # Wait for the friendly to clear rather than placing.
    if _facing_at_friendly_builder(best, direction):
        return
