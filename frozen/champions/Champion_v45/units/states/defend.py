...
def _claim_siege():
    """('screen', tile, turret) or ('kill', turret, turret) for a besieging turret.
    ...
    the kill when the shot cannot be screened — a sentinel, or a ray with no free
    tile left on it.
    """
...
def _run_siege() -> None:
    ...
    if adjacent and rc.can_fire(target) and rc.get_global_resources() >= 2:
        rc.fire(target)
        return
    nav.move_adjacent(target)
