...
import units.atk_states.sentinel_siege as sentinel_siege
import units.atk_states.sentinel_guard as sentinel_guard
import units.atk_states.core_sabotage as core_sabotage
import units.econ_states.park    as park
import units.econ_states.base_guard as base_guard
import units.econ_states.mirror_guard as mirror_guard
import units.econ_states.anti_sentinel as anti_sentinel
...
_ATK_STATES = tuple(sorted(
    [explore, sentinel_siege, sentinel_guard, core_sabotage, counter_mirror],
    key=lambda s: s.MAX_SCORE,
    reverse=True
))
...
_ECONOMY_STATES = tuple(sorted(
    [harvest, route, park, base_guard, anti_sentinel],
    key=lambda s: s.MAX_SCORE,
    reverse=True
))

# After an econ bot has finished a route (detected the first time it parks — park
# only scores once economy is delivering to the core and route/harvest have no
# work left), it is locked out of route and harvest for the rest of the game: it
# only defends (attack) or parks near the core.
_DONE_ECON_STATES = tuple(sorted(
    [park, base_guard, mirror_guard, anti_sentinel, counter_mirror],
    key=lambda s: s.MAX_SCORE,
    reverse=True
))
...
# The first opening builder is the sole attacker. It is launcher-inserted toward
# the known enemy core, places the two-sentinel siege, then guards it.
_atk_bot = False
...
# Latched True once this econ bot has finished a route (first time it parks).
# From then on it can never re-enter route or harvest (see _DONE_ECON_STATES).
_route_done = False
...
def init(c: Controller):
    ...
    sentinel_siege.init(c)
    sentinel_guard.init(c)
    core_sabotage.init(c)
    base_guard.init(c)
    mirror_guard.init(c)
    anti_sentinel.init(c)
...
def select_best_state():
    ...
    if _economy_builder:
        if assigned_econ_ore() is None or _assigned_route_complete():
            _route_done = True
        if _route_done and _economy_index in (0, 1):
            comms.mark_defender_ready(_economy_index)
    best_state = None
    best_score = 0
...
def atk_target():
    ...
    last = sentinel_siege.last_positions[-1] if sentinel_siege.last_positions else None
    if last is not None:
        my_team = map_info._bm_team[map_info._my_team_idx]
        if map_info._bm_et[map_info._IDX_SENTINEL] & my_team & (1 << (last.x + last.y * map_info._width)):
            adj = _nearest_open(
                (last.x + dx, last.y + dy)
...
def assigned_econ_ore() -> Position | None:
    """The ore deterministically assigned to this economy builder."""
    if not _economy_builder or _economy_index is None:
        return None
    ores = launch_plan.ranked_ore_from_core()
    return ores[_economy_index] if _economy_index < len(ores) else None


def _assigned_route_complete() -> bool:
    ore = assigned_econ_ore()
    if ore is None:
        return False
    ore_bit = 1 << (ore.x + ore.y * map_info._width)
    my_team = map_info._bm_team[map_info._my_team_idx]
    if not (ore_bit & my_team & map_info._bm_et[map_info._IDX_HARVESTER]):
        return False
    connected = map_info.route_reaches_core_mask()
    adjacent = map_info.expand_manhattan(ore_bit)
    return bool(adjacent & connected)
...
def run():
    ...
    # Opening transport: build/queue beside the core launcher and hold until it
    # flings this role toward its first objective.
...
def _log_status():
    ...
    if _chosen_state is not None:
        ...
        mode = getattr(_chosen_state, "_mode", "")
        if mode:
            state += ":" + mode
