...
def ti_reserve() -> float:
    return TI_RESERVE * _rc.get_scale_percent() / 100
...
_enemy_supply_cache_key: tuple | None = None
_enemy_supply_cache: int = 0


def enemy_conveyors_with_possible_supply() -> int:
    """Enemy conveyor-like tiles that may still have upstream titanium supply.

    A visible conveyor is proven disconnected only when a reverse flood-fill
    stays entirely inside current vision and reaches neither an enemy harvester
    nor an unseen cardinal predecessor tile. Reaching the edge of vision keeps
    the line valid because an unseen conveyor or harvester may feed it. Enemy
    conveyors outside current vision are likewise preserved as uncertain.

    The returned mask includes conveyors and splitters, matching
    ``_bm_conveyors`` and the placement scorer's conveyor-valued targets.
    """
    global _enemy_supply_cache_key, _enemy_supply_cache
    key = (_struct_version, _bm_visible)
    if key == _enemy_supply_cache_key:
        return _enemy_supply_cache

    enemy = _bm_team[1 - _my_team_idx]
    enemy_conveyors = _bm_conveyors & enemy
    if not enemy_conveyors:
        _enemy_supply_cache_key = key
        _enemy_supply_cache = 0
        return 0

    visible = _bm_visible
    # Anything not visible is uncertain by definition and must remain a target.
    possible = enemy_conveyors & ~visible
    enemy_harvesters = _bm_et[_IDX_HARVESTER] & enemy
    reverse = _conv_reverse
    w = _width

    targets = enemy_conveyors & visible
    while targets:
        target_bit = targets & -targets
        targets ^= target_bit
        frontier = target_bit
        visited = 0
        supplied_or_uncertain = False

        while frontier and not supplied_or_uncertain:
            current = frontier & -frontier
            frontier ^= current
            if visited & current:
                continue
            visited |= current
            n = current.bit_length() - 1

            # A remembered but currently unseen predecessor may have changed,
            # and its own feeder is unknown, so do not prove disconnection.
            if not (current & visible):
                supplied_or_uncertain = True
                break

            adjacent = expand_manhattan(current) & ~current & _board_mask
            if adjacent & enemy_harvesters & visible:
                supplied_or_uncertain = True
                break
            # Every possible conveyor/harvester input is cardinal. If even one
            # such tile is outside vision, upstream supply remains possible.
            if adjacent & ~visible:
                supplied_or_uncertain = True
                break

            predecessors = reverse[n] & enemy_conveyors & ~visited
            frontier |= predecessors

        if supplied_or_uncertain:
            possible |= target_bit

    _enemy_supply_cache_key = key
    _enemy_supply_cache = possible
    return possible
