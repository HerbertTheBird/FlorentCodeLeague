...
def _try_heal_core() -> bool:
    """Heal an adjacent damaged core tile, ignoring lower-priority buildings."""
    damaged_core = map_info._bm_my_core_area & map_info._bm_damaged
    if not damaged_core:
        return False
    w = map_info._width
    my_pos = map_info._my_pos
    ...
    while m:
        ...
        if my_pos.distance_squared(pos) != 1 or not rc.can_heal(pos):
            continue
        damage = map_info._MAX_HP_BY_IDX[map_info._IDX_CORE] - map_info._building_hp[n]
        if damage > best_damage:
            best_damage = damage
            best = pos
    if best is None:
        return False
    log("heal: core fallback", best)
    rc.heal(best)
    return True


def _enemy_within_core_dsq(target: Position, max_dsq: int) -> bool:
    core = map_info._my_core
    if core is None:
        return False
    for dx in (0, 1):
        for dy in (0, 1):
            if target.distance_squared(Position(core.x + dx, core.y + dy)) <= max_dsq:
                return True
    return False


def _try_core_guard(target: Position) -> bool:
    """Guard a close chased bot from a safe, cardinally core-adjacent tile."""
    if not _enemy_within_core_dsq(target, 4):
        return False

    w = map_info._width
    my_pos = map_info._my_pos
    my_bit = 1 << (my_pos.x + my_pos.y * w)
    core_adjacent = (map_info.expand_manhattan(map_info._bm_my_core_area)
                     & ~map_info._bm_my_core_area)
    core_adjacent &= map_info._board_mask & ~map_info.get_avoid(False)
    core_adjacent &= ~map_info._bm_enemy_turret_threat
    core_adjacent &= ~((map_info._bm_friendly_bots | map_info._bm_enemy_bots)
                       & ~my_bit)
    if not core_adjacent:
        return False
    ...
    while m:
        ...
        m ^= lsb
        pos = Position(n % w, n // w)
        _, travel = nav.closest(lsb)
        if travel < 0:
            continue
        key = (pos.distance_squared(target), travel, n)
    ...
    log("heal: core guard", target, "via", best)
    if best != my_pos:
        nav.move_to(best)
    _try_heal_core()
    return True
