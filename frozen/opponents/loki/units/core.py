...
def run():
    ...
    # by the core converting titanium 1:1 (at most once per turn). Target scales
    # with our army: builders bump the shared gunner counter (store slot 15,
    # comms.GUNNER_COUNT_SLOT) each time they place a gunner, and we keep the
    # pool topped up to 2 * (gunners + 1).
    ammo = rc.get_global_ammo()
    ammo_target = 2 * (comms.gunner_count() + 1)
    if ammo < ammo_target:
