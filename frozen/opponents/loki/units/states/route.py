...
def score():
    ...
    important = map_info.expand_manhattan(map_info._bm_enemy_bots, 5)&~(map_info._bm_team[map_info._my_team_idx]&map_info._bm_et[map_info._IDX_HARVESTER])
...
def run():
    ...
    if target_conveyor[0].distance_squared(target_conveyor[1]) == 1:
        ...
        if tc1_zone & map_info._bm_enemy_bots:
            near_enemy = True
    if near_enemy and map_info.team_at(target_conveyor[1].x, target_conveyor[1].y) != rc.get_team() and map_info.team_at(target_conveyor[0].x, target_conveyor[0].y) == rc.get_team():
