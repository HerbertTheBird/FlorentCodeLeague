...
class Player:
    ...
    def _try_move(self, ct: Controller, d: Direction) -> bool:
        ...
        # Bounds guard: is_tile_empty() raises GameError off-map, and at a map
        # edge next_pos can point outside the board. Skip such directions.
        if not (0 <= next_pos.x < ct.get_map_width() and 0 <= next_pos.y < ct.get_map_height()):
            return False
